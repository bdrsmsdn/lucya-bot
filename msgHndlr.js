...
const { liriklagu, quotemaker, randomNimek, sleep, jadwalTv, ss, stickerburn, stickerlight, nhentai, instagram, igstory, missin, custome } = require('./lib/functions')
const { admeen, snk, info, donate, readme, listChannel, prem, sewa, nsfwmenu, maker, media, edukasi, funmenu, poll, weebs, tools, infor, anonymous, contact, tnc, others } = require('./lib/help')
...
const { downloader, urlShortener, cekResi, images, rugapoi, nobg, nsfww } = require('./lib')
...
// FUNCTION
                    ...
                    for (let i of limit){
                        if(i.id === id){
                            ...
                            if (limits >= limitCount) {
                                found = true;
                                aksa.reply(from, 'Perintah BOT anda sudah mencapai batas, coba esok hari :)', id)
                                return true;
                            }else{
                                limit
                                found = true;
                ...
                for (let i of medialimit) {
                    if (i.id === id) {
                        ...
                        if (limitsa >= medialimitCount) {
                            found = true;
                            aksa.reply(from, 'Perintah media anda sudah mencapai batas, coba esok hari atau coba perintah lain:)', id)
                            return true;
                        } else {
                            medialimit
                            found = true;
            ...
            if (isGroupMsg && isBadWord) {
                if (bad.includes(chats)){
                    //if (!isBadWord) return aksa.reply(from, 'Fitur ANTI BADWORD belum Aktif Pak', id)
                    if (!isGroupAdmins) {
                        return aksa.reply(from, "JAGA UCAPAN DONG!! 😠", id)
                            .then(() => aksa.removeParticipant(groupId, sender.id))
                            .then(() => {
                                aksa.sendText(from, `*「 ANTI BADWORD 」*\nKamu dikick karena berkata kasar!`)
                            }).catch(() => aksa.sendText(from, `Untung cya bukan admin, kalo admin udah cya kick!`))
                    } else {
                        return aksa.reply(from, "Tolong Jaga Ucapan Min 😇", id)
                    }
                }
            }
                ...
                if(body === `${prefix}mute` && isMuted(chatId) == true){
                    if(isGroupMsg) {
                        if (!isGroupAdmins) return aksa.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh admin Lucya!', id)
                        ...
                        aksa.reply(from, `Bot telah di mute pada chat ini! ${prefix}unmute untuk unmute`,id)
                    }else{
                        if(isMsgLimit(serial)){
                            return
                        }else{
                ...
                if(body === `${prefix}unmute` && isMuted(chatId) == false){
                    if(isGroupMsg) {
                        ...
                        fs.writeFileSync('./lib/muted.json', JSON.stringify(muted, null, 2))
                        aksa.reply(from, 'Bot telah di unmute!', id)         
                    }else{
                        if(isMsgLimit(serial)){
                            return
                        }else{
                ...
                aksa.reply(from, 'Iya ada apa?', id)
            }
                ...
                aksa.reply(from, `sama-sama ${pushname}💖`, id)
            }
                if (body === `${prefix}unbanchat`) {
                    if (!isOwner) return aksa.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh Owner Lucya!', id)
                    if(setting.banChats === false) return
                    setting.banChats = false
                    banChats = false
                    fs.writeFileSync('./lib/setting.json', JSON.stringify(setting, null, 2))
                    aksa.reply(from, 'Global chat has been disable!', id)
                }
        ...
        case `${prefix}banchat`:
            ...
            if (!isOwner) return aksa.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Lucya!', id)
            setting.banChats = true
            banChats = true
            fs.writeFileSync('./lib/setting.json', JSON.stringify(setting, null, 2))
            aksa.reply(from, 'Global chat has been enable!', id)
            break
		case `prefix`:
			aksa.reply(from, `*Lucya is Use ( ${prefix} ) Prefix!.* 
_Prefix adalah tanda di awal perintah._
_Contoh: ${prefix}menu_`, id)
			break
		case `${prefix}setprefix`:
			if (!isOwner) return aksa.reply(from, 'Maaf, Fitur ini hanya untuk OWNER dan ADMIN Lucya!', id)
			if (args.length === 1) return aksa.reply(from, `*Kirim Perintah ${prefix}setto [prefix baru]*. 
Contoh: ${prefix}setprefix #`, id)
			const pf = body.slice(11)  
			setting.prefix = `${pf}`
			prefix = `${pf}`
			fs.writeFileSync('./lib/setting.json', JSON.stringify(setting, null,2))
			aksa.reply(from, `Change Prefix To ${pf} SUCCESS!`, id)
            break
        case `${prefix}addbadword`:
            if (!isOwner) return aksa.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Lucya!', id)
			if (!isGroupAdmins) return aksa.reply(from, 'Perintah ini hanya bisa di gunakan oleh Admin group', id)
			if (args.length == 1) return aksa.reply(from, `Kirim perintah ${prefix}addbadword [kata kasar]. contoh ${prefix}addbadword bego`,id)
                const bw = body.slice(12)
				bad.push(bw)
			fs.writeFileSync('./lib/bad.json', JSON.stringify(bad))
                aksa.reply(from, 'Success Menambahkan Bad Word!', id)
				break
		case `${prefix}delbadword`:
            if (!isOwner) return aksa.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Lucya!', id)
			if (!isGroupAdmins) return aksa.reply(from, 'Perintah ini hanya bisa di gunakan oleh Admin group', id)
			if (args.length == 1) return aksa.reply(from, `Kirim perintah ${prefix}delbadword [kata kasar]. contoh ${prefix}delbadword bego`,id)
				let dbw = body.slice(12)
				bad.splice(dbw)
				fs.writeFileSync('./lib/bad.json', JSON.stringify(bad))
				aksa.reply(from, 'Success Menghapus BAD WORD!', id)
			break
        case `${prefix}listbanned`:
            ...
            await aksa.reply(from, bened, id)
            break
        case `${prefix}block`:
            if (!isOwner) return aksa.reply(from, 'Perintah ini hanya bisa digunakan oleh Owner', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
                let block = `${mentionedJidList[i]}`
                await aksa.contactBlock(block).then((a) => {
                    console.log(a)
                    aksa.reply(from, `Success block ${args[1]}!`, id)
                })
            }
            break
        case `${prefix}blok`: //bedanya apa? kalo blok ketik nomornya gausah ditag
            ...
            if (args.length >= 2) {
                ...
                await aksa.contactBlock(block).then(() => {
                    return aksa.reply(from, `Sukses blok ${args[1]}!`, id)
                })
                return
            }
            break
        case `${prefix}unblock`:
            ...
            for (let i = 0; i < mentionedJidList.length; i++) {
                ...
                await aksa.contactUnblock(unblock).then((a) => {
                    console.log(a)
                    aksa.reply(from, `Success unblok ${args[1]}!`, id)
                })
            }
            break
        case `${prefix}unblok`:
            ...
            if (args.length >= 2) {
                ...
                await aksa.contactBlock(unblock).then(() => {
                    aksa.reply(from, `Sukses unblok ${args[1]}!`, id)
                })
                return
            }
            break
        case `${prefix}cgc`:
            ...
            aksa.sendText(from, 'Berhasil membuat group!')
            break
        case `${prefix}setgrouplimit`:
            if (!isOwner) return aksa.reply(from, 'Perintah ini hanya untuk Owner Lucya!', id)
            if (args.length == 1) return aksa.reply(from, `Kirim perintah *!setgrouplimit ['!']*, contoh *!setgrouplimit 15*`, id)
            const gli = body.slice(15)
            setting.groupLimit = `${gli}`
            groupLimit = `${gli}`
            fs.writeFileSync('./lib/setting.json', JSON.stringify(setting, null, 2))
            aksa.reply(from, `Mengubah limit group ke ${gli} silakan restart!`, id)
            break
        case `${prefix}seticon`: 
			...
			if (!isGroupMsg) return aksa.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
            if (!isGroupAdmins) return aksa.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
            if (!isBotGroupAdmins) return aksa.reply(from, 'Gagal, silahkan tambahkan Lucya sebagai admin grup!', id)
			...
			if (isMedia && type == 'image' || isqwtimg) {
				...
				if (!isUrl(url)) { await aksa.reply(from, 'Maaf, link yang kamu kirim tidak valid.', id) }
				aksa.setGroupIconByUrl(groupId, url).then((r) => (!r && r !== undefined)
				? aksa.reply(from, 'Maaf, link yang kamu kirim tidak memuat gambar.', id)
				: aksa.reply(from, 'Berhasil mengubah profile group', id))
			} else {
				aksa.reply(from, `Commands ini digunakan untuk mengganti icon/profile group chat\n\n\nPenggunaan:\n1. Silahkan kirim/reply sebuah gambar dengan caption !seticon\n\n2. Silahkan ketik !seticon linkImage`)
			}
            break
        case `${prefix}setpp`:
            if(!isOwner) return aksa.reply(from, 'Perintah ini hanya untuk Owner!', id)
			...
			if (isMedia && type == 'image' || isqwtimg1) {
                ...
                await aksa.reply(from, 'Berhasil mengubah foto profile!', id)
            }
            break
        case `${prefix}setname`:
            if (!isOwner) return aksa.reply(from, `Perintah ini hanya bisa di gunakan oleh Owner Lucya!`, id)
            ...
            aksa.sendTextWithMentions(from, `Makasih Nama Barunya @${sender.id.replace('@c.us','')} 😘`, id)
            break
        case `${prefix}setstatus`:
            if (!isOwner) return aksa.reply(from, `Perintah ini hanya bisa di gunakan oleh Owner Lucya!`, id)
            const setstat = body.slice(11)
            await aksa.setMyStatus(setstat)
            aksa.sendTextWithMentions(from, `Makasih Status Barunya @${sender.id.replace('@c.us','')} 😘`, id)
            break
        case `${prefix}setgroupname`:
            if (!isGroupMsg) return aksa.reply(from, `Fitur ini hanya bisa di gunakan dalam group`, id)
            if (!isGroupAdmins) return aksa.reply(from, `Fitur ini hanya bisa di gunakan oleh admin group`, id)
            if (!isBotGroupAdmins) return aksa.reply(from, `Fitur ini hanya bisa di gunakan ketika bot menjadi admin`, id)
            const namagrup = body.slice(14)
            let sebelum = chat.groupMetadata.formattedName
            let halaman = global.page ? global.page : await aksa.getPage()
            await halaman.evaluate((chatId, subject) =>
            Store.WapQuery.changeSubject(chatId, subject),groupId, `${namagrup}`)
            aksa.sendTextWithMentions(from, `Nama group telah diubah oleh admin @${sender.id.replace('@c.us','')}\n\n• Before: ${sebelum}\n• After: ${namagrup}`)
            break
        case 'tes':
            if (isOwner) return aksa.reply(from, 'Ok', id)
            break
        case `${prefix}leaveall`:
            ...
            aksa.reply(from, 'Succes leave all group!', id)
            break
        case `${prefix}clearall`:
            ...
            aksa.reply(from, 'Succes clear all chat!', id)
            break
        case `${prefix}join`:
            if (!isOwner) return aksa.reply(from, 'Silakan hubungi Owner untuk mengundang Lucya ke dalam Grup. Ketik !owner', id)
            if (args.length < 2) return aksa.reply(from, 'Silakan hubungi Owner untuk mengundang bot ke dalam Grup. Ketik !owner', id)
            const link = args[1]
            const tGr = await aksa.getAllGroups()
            const minMem = 2
            const check = await aksa.inviteInfo(link)
            const isLink = link.match(/(https:\/\/chat.whatsapp.com)/gi)
            if (!isLink) return aksa.reply(from, 'Ini link? 👊🤬', id)
            if (tGr.length > 50) return aksa.reply(from, 'Maaf jumlah group sudah maksimal!', id)
            if (check.size < minMem) return aksa.reply(from, 'Member group tidak melebihi 2, Lucya tidak bisa masuk', id)
            if (check.status === 200) {
                await aksa.joinGroupViaLink(link).then(() => aksa.reply(from, 'Lucya akan segera masuk!'))
            } else {
                aksa.reply(from, 'Link group tidak valid!', id)
            }
            break
        case `${prefix}mtcstart`:
            ...
            setting.mtc = true
            fs.writeFileSync('./lib/setting.json', JSON.stringify(setting, null, 2))
            aksa.reply(from, 'Maintenance sudah di Umumkan!', id)
            break
        case `${prefix}mtcstop`:
...
❉──────────────────❉\n\n${bctxt}`
            ...
            break
        case `${prefix}getses`:
            if (!isOwner) return aksa.reply(from, 'Fitur ini hanya untuk Owner Lucya!, id')
            const sesPic = await aksa.getSnapshot()
            aksa.sendFile(from, sesPic, 'session.png', 'Nih...', id)
            break
        ...
        case `${prefix}listblok`:
            ...
            aksa.sendTextWithMentions(from, hih, id)
            break
...
❉──────────────────❉\n`
            ...
            break
        case `${prefix}ban`:
            if (!isOwner) return aksa.reply(from, 'Hanya Owner Lucya yang bisa!:p', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
                banned.push(mentionedJidList[i])
                fs.writeFileSync('./lib/banned.json', JSON.stringify(banned))
                aksa.reply(from, 'Succes ban target!', id)
            }
            break
        case `${prefix}unban`:
            if (!isOwner) return aksa.reply(from, 'Hanya Owner Lucya yang bisa:p', id)
            ...
            fs.writeFileSync('./lib/banned.json', JSON.stringify(banned))
            aksa.reply(from, 'Unbanned User!', id)
            break

            //primary menu-----------------------------------------------------------------------------------------------

        case `${prefix}regis`: //AKSARA 
            if (isRegis) return tobz.reply(from, 'Anda sudah melakukan registrasi!', id)
            if (args === 1) return tobz.reply(from, `Silakan isi dengan nama dan nomor telepon! Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            arg = body.trim().split('|')
            const nimi = arg[1]
            const nimir = arg[2]
...
Total user terdaftar : ${regis.length}
          ...
          ║▌│█║▌│ █║▌│█│║▌║
                           ...
                           *_AKSARA_*` : false
            if (!hasilll) return tobz.reply(from, 'Nomor WhatsApp tidak valid [ Tidak terdaftar di WhatsApp ] atau gunakan 62 di awal bukan 0', id)
            regis.push(mentahh.id._serialized)
            fs.writeFileSync('./lib/regis.json', JSON.stringify(regis))
            tobz.sendText(from, hasilll)
            }
            break
        case `${prefix}unreg`: //menghapus nomor dari database
            if (!isOwner) return aksa.reply(from, 'Fitur ini hanya dapat digunakan oleh Owner Lucya')
            if (args.length === 1) return aksa.reply(from, 'Masukkan nomornya, *GUNAKAN AWALAN 62* contoh: 6281281817375')
            let inx = regis.indexOf(args[1] + '@c.us')
            regis.splice(inx, 1)
            fs.writeFileSync('./lib/regis.json', JSON.stringify(regis))
            aksa.reply(from, 'Sukses menghapus nomor from database', id)
            break
        case `${prefix}limit`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (isAdmin) return aksa.reply(from, `Sisa limit request anda tersisa : *UNLIMITED*\n\n_Note : Limit akan direset setiap jam 21:00!_`, id)
            var found = false
            const limidat = JSON.parse(fs.readFileSync('./lib/limit.json'))
            for (let lmt of limidat) {
                if (lmt.id === serial) {
                    let limitCounts = limitCount - lmt.limit
                    if (limitCounts <= 0) return aksa.reply(from, `Limit request anda sudah habis\n\n_Note : Limit akan direset setiap jam 21:00!_`, id)
                    aksa.reply(from, `Sisa limit request anda tersisa : *${limitCounts}*\n\n_Note : Limit akan direset setiap jam 21:00!_`, id)
                    found = true
                }
            }
            //console.log(limit)
        ...
        case `${prefix}limed`: //batesin biar ga kebanyakan download fitur youtube:D
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (isAdmin) return aksa.reply(from, `Sisa limit request media anda tersisa : *UNLIMITED*\n\n_Note : Limit akan direset setiap jam 21:00!_`, id)
            var found = false
            ...
            for (let lmta of limed) {
                if (lmta.id === serial) {
                    ...
                    if (medialimitCounts <= 0) return aksa.reply(from, `Limit request media anda sudah habis\n\n_Note : Limit akan direset setiap jam 21:00!_`, id)
                    aksa.reply(from, `Sisa limit request media anda tersisa : *${medialimitCounts}*\n\n_Note : Limit akan direset setiap jam 21:00!_`, id)
                    found = true
                }
            }
            //console.log(limit)
            //console.log(limidat)
            if (found === false) {
                let obj = { id: `${serial}`, medialimit: 1 };
                medialimit.push(obj);
...
`, id)
        ...
        case `${prefix}menugroup`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (!isGroupMsg) return aksa.reply(from, 'Maaf, perintah ini hanya dapat digunakan didalam grup!', id)
            if (!isGroupAdmins) return aksa.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
            aksa.sendText(from, admeen(), id)
            break
        case `${prefix}menuprem`:
        case `${prefix}vip`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (!isAdmin) return aksa.reply(from, 'Mohon maaf anda tidak bisa menggunakan fitur premium!', id)
            aksa.sendText(from, prem(), id)
            break
        case `${prefix}maker`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            aksa.reply(from, maker(), id)
            break
        case `${prefix}media`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            aksa.reply(from, media(), id)
            break
        case `${prefix}edukasi`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            aksa.reply(from, edukasi(), id)
            break
        case `${prefix}funmenu`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            aksa.reply(from, funmenu(), id)
            break
        case `${prefix}iklan`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            aksa.sendLinkWithAutoPreview(from, 'https://instagram.com/kayora.id', sewa())
            break
        case `${prefix}poll`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            aksa.reply(from, poll(), id)
            break
        case `${prefix}pray`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            aksa.reply(from, pray(), id)
            break
        case `${prefix}weebs`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            aksa.reply(from, weebs(), id)
            break
        case `${prefix}tools`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            aksa.reply(from, tools(), id)
            break
        case `${prefix}contact`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            aksa.reply(from, contact(), id)
            break
        case `${prefix}anonymous`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            aksa.reply(from, anonymous(), id)
            break
        case `${prefix}information`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            aksa.reply(from, infor(), id)
            break
        case `${prefix}tnc`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            aksa.reply(from, tnc(), id)
            break
        case `${prefix}others`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            aksa.reply(from, others(), id)
            break
        case `${prefix}readme`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            aksa.reply(from, readme(), id)
            break
        case `${prefix}info`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            aksa.sendLinkWithAutoPreview(from, 'https://instagram.com/bdrsmsdn', info(), id)
            break
        case `${prefix}snk`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            5
            aksa.reply(from, snk(), id)
            break
        case `${prefix}donasi`:
        case `${prefix}donate`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            aksa.reply(from, donate(), id)
            break
        case `${prefix}linkwa`:
            aksa.reply(from, `wa.me/${serial.replace(/@c.us/g,'')}`, id)
            break
        case `${prefix}cekprem`:
            if (!isAdmin) return aksa.reply(from, 'Anda bukan Member Premium, silakan hubungi owner untuk membeli akses Premium!', id)
            ...
            aksa.sendFileFromUrl(from, pfpp, 'photo.jpg', `
╭────「 *PREMIUM👑* 」──
│+ *Name* : ${pushname}
...
*Provider :* INDIHOME ASU`, id)
        ...
        case `${prefix}stgif`:
            ...
            aksa.sendStickerfromUrl(from, "https://api.vhtear.com/textxgif?text=" + quoteText + "&apikey=" + vhtear)
            } else {
            const bjbjbj = body.slice(7)
            aksa.sendStickerfromUrl(from, "https://api.vhtear.com/textxgif?text=" + bjbjbj + "&apikey=" + vhtear)
            }
            break
        ...
        case `${prefix}sgif`: // MRHRTZ
            ...
            await limitAdd(serial)
            aksa.reply(from, `*Tunggu!* Permintaan Anda sedang diproses \n\n-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-\n\n${donate()}`, id)
            if (isMedia && type === 'video' || mimetype === 'image/gif') {
            try {
                const mediaData = await decryptMedia(message, uaOverride)
                await aksa.sendMp4AsSticker(from, mediaData, {fps: 15, startTime: `00:00:00.0`, endTime : `00:00:05.0`,loop: 0})
                } catch (e) {
                    aksa.reply(from, `Size media terlalu besar! mohon kurangi durasi video.`, id)
                }
            } else if (quotedMsg && quotedMsg.type == 'video' || quotedMsg && quotedMsg.mimetype == 'image/gif') {
                const mediaData = await decryptMedia(quotedMsg, uaOverride)
                await aksa.sendMp4AsSticker(from, mediaData, {fps: 15, startTime: `00:00:00.0`, endTime : `00:00:05.0`,loop: 0})
            } else {
                aksa.reply(from, `Kesalahan ⚠️ Hanya bisa video/gif apabila file media berbentuk gambar ketik #sticker`, id)
            } 
            break
        ...
        case `${prefix}snbg`:
            ...
            if (isMedia) {
                try {
                    nobg.Removebg()
                    aksa.sendFileFromUrl(from, image, id)
                } catch (err) {
                    console.log(err)
                }
            }
        ...
        case `${prefix}stickerfire`:
            ...
            if (isMedia && type === 'image' || isQuotedImage) {
                ...
                await aksa.sendStickerfromUrl(from, Sfire)
            } else {
                await aksa.reply(from, 'Wrong Format!\n⚠️ Harap Kirim Gambar Dengan !stickerfire', id)
            }
            break
        case `${prefix}stgif2`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            ...
            aksa.sendFileFromUrl(from, "https://api.vhtear.com/slidingtext?text="+ eqw +"&apikey=" + vhtear)
            break
        case `${prefix}stickerlight`:
            ...
            if (isMedia && type === 'image' || isQuotedImage) {
                ...
                await aksa.sendStickerfromUrl(from, slight)
            } else {
        ...
        case `${prefix}s2img`:
            ...
            if (quotedMsg && quotedMsg.type == 'sticker') {
                ...
                aksa.reply(from, `*Tunggu!* Permintaan Anda sedang diproses \n\n-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-\n\n${donate()}`)
                const imageBase64 = `data:${quotedMsg.mimetype};base64,${mediaData.toString('base64')}`
                await aksa.sendFile(from, imageBase64, 'imagesticker.jpg', 'nih', id)
            } else if (!quotedMsg) return aksa.reply(from, 'Mohon tag sticker yang ingin dijadikan gambar!', id)
            break
        case `${prefix}ttp`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (!isAdmin) return aksa.reply(from, 'Mohon maaf anda tidak bisa menggunakan fitur premium!', id)
            if (quotedMsg){
            const quoteText = quotedMsg.type == 'chat' ? quotedMsg.body : quotedMsg.type == 'image' ? quotedMsg.caption : ''
            aksa.sendStickerfromUrl(from, `https://api.vhtear.com/textmaker?text=${quoteText}&warna=white&apikey=${vhtear}`)
            } else {
            const bjbjbjaa = body.slice(5)
            aksa.sendStickerfromUrl(from, `https://api.vhtear.com/textmaker?text=${bjbjbjaa}&warna=white&apikey=${vhtear}`)
            }
            break
        case `${prefix}musik`:
        case `${prefix}music`:
        ...
        case `${prefix}movie`:
            ...
            try {
                ...
                const { rating, sinopsis, thumb, title, video } = await mova.result
                const movi = `*「 MOVIE 」*\n\n*Hasil Pencarian Film: ${mov}*\n\n*Rating* : ${rating}\n*Title* : ${title}\n*Sinopsis* : ${sinopsis}\n*Link* : ${video}`
                aksa.sendFileFromUrl(from, thumb, 'thumbjpg', movi, id)
            }catch (err) {
                aksa.sendText(ownerNumber, 'Error Movie : '+ err)
                aksa.reply(from, 'Error!', id)
            }
            break 
        case `${prefix}nhentai`:
            //premium command, lu tau kemana harus ngehubungi
        ...
        case `${prefix}lewd`:
            ...
            await limitAdd(serial)
            if (isGroupMsg) {
                if (!isNsfw) return await aksa.reply(from, 'NSFW belum diaktifkan di grup ini!', id)
                await aksa.reply(from, mess.wait, id)
                nsfww.randomLewd()
                    .then(async ({ url }) => {
                        await aksa.sendFileFromUrl(from, url, 'lewd.jpg', '', null, null, true)
                            .then(() => console.log('Success sending lewd!'))
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await aksa.reply(from, err, id)
                    })
            } else {
                await aksa.reply(from, mess.wait, id)
                nsfww.randomLewd()
        ...
        case `${prefix}fetish`: //thanks to BOCCHI BOT
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (!isAdmin) return aksa.reply(from, 'Mohon maaf anda tidak bisa menggunakan fitur premium!', id)
            if (args.length === 1) return await aksa.reply(from,'Wrong Format!', id)
            if (isGroupMsg) {
                if (!isNsfw) return await aksa.reply(from, 'NSFW belum diaktifkan di grup ini!', id)
                await aksa.reply(from, mess.wait, id)
                try {
        ...
        case `${prefix}ptlvid`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if(!isAdmin) return aksa.reply(from, 'Perintah ini hanya untuk Member Premium',id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik !limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            aksa.sendFileFromUrl(from, rindKiya, 'asupan.mp4', 'Nih', id)
            break
        ...
        case `${prefix}phdl`: //thanks to BOCCHI BOT
            ...
            if (!isAdmin) return aksa.reply(from, 'Mohon maaf anda tidak bisa menggunakan fitur premium!', id)
            if (isGroupMsg) {
                if (!isNsfw) return await aksa.reply(from, ind.notNsfw(), id)
                if (!args[1].match(isUrl) && !args[1].includes('instagram.com')) return aksa.reply(from, `Maaf, link yang kamu kirim tidak valid. [Invalid Link]`, id)
                await aksa.reply(from, mess.wait, id)
                try {
                    nsfww.phDl(yuerel)
                        .then(async ({ title, download_urls, thumbnail_url }) => {
                            ...
                            if (count !== 2) {
                                ...
                                await aksa.sendFileFromUrl(from, thumbnail_url, `${title}`, `Title: ${title}\n\nLinks:\n${shortsLow} (240P)\n${shortsMid} (480P)\n${shortsHigh} (720P)`, id)
        ...
        case `${prefix}giftlimit`: //thanks to SASHA BOT
            if (!isAdmin) return aksa.reply(from, 'Perintah ini hanya untuk Owner dan Member Premium!', id)
            var found = false;
                    Object.keys(limit).forEach((i) => {
                    ...
                    aksa.sendTextWithMentions(from, `menambahkan ${args[1]} limit ke @${mentionedJidList[0].replace('@c.us', '')}` )
                break
            case `${prefix}google`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (!isAdmin) return aksa.reply(from, 'Mohon maaf anda tidak bisa menggunakan fitur premium!', id)
            ...
            if (googleQuery == undefined || googleQuery == ' ') return aksa.reply(from, `*Hasil Pencarian : ${googleQuery}* tidak ditemukan`, id)
            google({ 'query': googleQuery }).then(results => {
                let vars = `_*Hasil Pencarian : ${googleQuery}*_\n`
                for (let i = 0; i < results.length; i++) {
                    vars += `\n═════════════════\n\n*Judul* : ${results[i].title}\n\n*Deskripsi* : ${results[i].snippet}\n\n*Link* : ${results[i].link}\n\n`
                }
                aksa.reply(from, vars, id);
                }).catch(e => {
                    console.log(e)
            ...
            case `${prefix}memestiker`:
                arg = body.trim().split('|')
                aksa.reply(from, 'Sedang di proses silahkan tunggu ± 1 min!', id)
                if ((isMedia || isQuotedImage) && args.length >= 2) {
                    ...
                    aksa.sendImageAsSticker(from, ImageBase64, 'image.png', '', null, true)
                        .catch(() => {
                            aksa.reply(from, 'Ada yang error!')
                        })
                } else {
                    await aksa.reply(from, `Tidak ada gambar! Silahkan kirim gambar dengan caption ${prefix}memesticker <teks_atas> | <teks_bawah>\ncontoh: ${prefix}meme teks atas | teks bawah`, id)
                }
                break
        /*case `${prefix}ttp`:
                if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
                ...
                try
                {
                    ...
                    if(args)
                    {
                        if(quotedMsgObj == null)
                        {
                            ...
                            {
                                try{
                                    await aksa.sendImageAsSticker(from, gasMake.base64)
                                }catch(err) {
                                    await aksa.reply(from, 'Gagal membuat.', id)
                                } 
                            }else{
                                await aksa.reply(from, gasMake.reason, id)
                            }
                        ...
                        await aksa.reply(from, 'Tidak boleh kosong.', id)
                    }
                }catch(error)
                {
                    console.log(error)
        ...
        case `${prefix}s`:
            ...
            if (isMedia && type === 'image') {
                ...
                await aksa.sendImageAsSticker(from, imageBase64)
                ...
                const url = args[1]
                if (url.match(isUrl)) {
                    await aksa.sendStickerfromUrl(from, url, { method: 'get' })
                        .catch(err => console.log('Caught exception: ', err))
                } else {
                    aksa.reply(from, mess.error.Iv, id)
                }
            } else {
                aksa.reply(from, mess.error.St, id)
            }
            break
        case `${prefix}esticker`: //aksara
            ...
            aksa.sendStickerfromUrl(from, "https://api.vhtear.com/emojitopng?code="+ bjbjbja +"&apikey=" + vhtear)
            break
        case `${prefix}sth`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik !limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            if (stha.length > 10) return aksa.reply(from, 'Teksnya kepanjangan sayang', id)
            aksa.reply(from, mess.wait, id)
            aksa.sendFileFromUrl(from, "https://api.vhtear.com/hartatahta?text="+ stha +"&apikey=" + vhtear)
            break
        case `${prefix}bp`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik !limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            if (sthaa.length > 10) return aksa.reply(from, 'Teksnya kepanjangan sayang', id)
            aksa.reply(from, mess.wait, id)
            aksa.sendFileFromUrl(from, "https://api.vhtear.com/blackpinkicon?text="+ sthaa +"&apikey=" + vhtear)
            break
        case `${prefix}thunder`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik !limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            aksa.sendFileFromUrl(from, "https://api.vhtear.com/thundertext?text="+ sthaas +"&apikey=" + vhtear)
            break
        case `${prefix}ff`: //thanks jojo 
            ...
            await limitAdd(serial)
            arg = body.trim().split('|')
            if (args === 1) return aksa.reply(from, 'Silakan gunakan !epep <teks>', id)
            const burik = await get.get(`https://api-jojo.herokuapp.com/api/epep?text=${args[1]}`).json()
            aksa.sendFileFromUrl(from, burik.result, id)
            break
        case `${prefix}phub`: //textmaker api from MRHRTZ
            ...
            arg = body.trim().split('|')
                const porn = arg[1]
                const hub = arg[2]
            if (args === 1) return aksa.reply(from, 'Silakan gunakan !phub |<teks>|<teks2>', id)
            const pron = await get.get(`http://nzcha-api.herokuapp.com/styletext/phub?text1=${porn}&text2=${hub}`).json()
            aksa.sendFileFromUrl(from, pron.result, id)
            break
        case `${prefix}glow`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik !limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            aksa.sendFileFromUrl(from, graffitystrz.result, id)
            break
        case `${prefix}joker`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik !limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            aksa.sendFileFromUrl(from, joki.result, id)
            break
        case `${prefix}codmw`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik !limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            aksa.sendFileFromUrl(from, jokia.result, id)
            break
        case `${prefix}avenger`:
        case `${prefix}avengers`:
            ...
            arg = body.trim().split('|')
                const spa = arg[1]
                const acea = arg[2]
            if (args === 1) return aksa.reply(from, 'Silakan gunakan !avengers |<teks>|<teks2>', id)
            const pronaa = await get.get(`http://nzcha-api.herokuapp.com/styletext/avenger?text1=${spa}&text2=${acea}`).json()
            aksa.sendFileFromUrl(from, pronaa.result, id)
            break
        case `${prefix}outline`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik !limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            aksa.sendFileFromUrl(from, jokiqa.result, id)
            break
        case `${prefix}sunset`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik !limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            aksa.sendFileFromUrl(from, jokiqas.result, id)
            break
        case `${prefix}minion`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik !limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            aksa.sendFileFromUrl(from, jokiqass.result, id)
            break
        case `${prefix}space`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik !limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            arg = body.trim().split('|')
                const sp = arg[1]
                const ace = arg[2]
            if (args === 1) return aksa.reply(from, 'Silakan gunakan !space |<teks>|<teks2>', id)
            const prona = await get.get(`http://nzcha-api.herokuapp.com/styletext/space?text1=${sp}&text2=${ace}`).json()
            aksa.sendFileFromUrl(from, prona.result, id)
            break
        case `${prefix}glitch`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik !limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            arg = body.trim().split('|')
                const spq = arg[1]
                const aceq = arg[2]
            if (args === 1) return aksa.reply(from, 'Silakan gunakan !glitch |<teks>|<teks2>', id)
            const pronaq = await get.get(`http://inyourdream.herokuapp.com/glitch?kata1=${spq}&kata2=${aceq}`).json()
            aksa.sendFileFromUrl(from, pronaq.status, id)
            break
        case `${prefix}blood`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik !limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            aksa.sendFileFromUrl(from, jokiqassa.result, id)
            break
        case `${prefix}lg`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik !limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            await limitAdd(serial)
            if (args === 1) return aksa.reply(from, 'Silakan gunakan !lg <teks>', id)
            const gggg = await get.get(`https://api-jojo.herokuapp.com/api/gaming?text=${args[1]}`).json()
            aksa.sendFileFromUrl(from, gggg.result, id)
            break
        case `${prefix}qrcode`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik !limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            await aksa.sendFileFromUrl(from, `https://api.qrserver.com/v1/create-qr-code/?size=500x500&data=${qrcodes}`, 'gambar.png', 'Process sukses!')
            break
        case `${prefix}quotemaker`:
            ...
            if (arg.length >= 4) {
                ...
                await quotemaker(quotes, author, theme).then(amsu => {
                    aksa.sendFile(from, amsu, 'quotesmaker.jpg', 'nih').catch(() => {
                        aksa.reply(from, mess.error.Qm, id)
                    })
                })
            } else {
                aksa.reply(from, 'Usage: \n!quotemaker |teks|watermark|theme\n\nEx :\n!quotemaker |ini contoh|bdr|random', id)
            }
            break
        case `${prefix}textmaker`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (!isGroupMsg) return aksa.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik !ceklimit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            if ((isMedia || isQuotedImage) && args.length >= 2) {
                ...
                await aksa.sendFile(from, ImageBase64, 'image.png', 'neh...')
            } else {
                await aksa.reply(from, 'Wrong Format!', id)
            }
            break
        case `${prefix}text2img`:
            ...
            await limitAdd(serial)
            if (args.length === 1) return aksa.reply(from, 'Kirim perintah *!text2img <teks>*. Jika kesulitan silakan lihat contoh di *!readme*', id)
            ...
            if (txt2img.error) {
                ...
                aksa.sendFileFromUrl(from, txt2img.result, '02726161.png', null, id)
            }
            break
        case `${prefix}sandwriting`:
            ...
            await limitAdd(serial)
            if (args.length === 1)  return aksa.reply(from, 'Kirim perintah #sandwriting [ Teks ]\nContoh #sandwriting MRG3P5 GANS', id)
   ...
   Text : ${swrt}`
            ...
            aksa.sendFileFromUrl(from, base64, swrt3)
          } catch (err) {
            console.error(err.message)
            await aksa.sendFileFromUrl(from, errorurl2, 'error.png', '💔 Maaf, User tidak ditemukan')
        ...
        case `${prefix}`:
            if (!isSimi) return aksa.reply(from, 'command/Perintah Simi belum di aktifkan! ketik !simi enable', id)
            if (args.length === 1) return aksa.reply(from, 'Kirim perintah *! [teks]*\nContoh : *! halo*')
            const que = body.slice(1)
            const sigo = await axios.get(`http://simsumi.herokuapp.com/api?text=${que}&lang=id`)
            const sigot = sigo.data
            aksa.reply(from, sigot.success, id)
            console.log(sigot)
            break
        case `${prefix}profile`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            ...
            if (isGroupMsg) {
                if (!quotedMsg) {
                    ...
                    await aksa.sendFileFromUrl(from, pfp, 'pfp.jpg',
                        `*User Profile* ✨️
➸ *Username 🐷 : ${namae}*
➸ *User Info 💫 : ${status}*
...
➸ *Admin Group 🔰 : ${adm ? 'Yes' : 'No'}*`)
        ...
        case `${prefix}groupinfo`:
            if (!isGroupMsg) return aksa.reply(from, 'Perintah ini hanya bisa digunakan dalam group!', message.id)
            ...
            await aksa.sendFileFromUrl(from, pfp, 'group.png', `➸ *Name : ${groupname}* 
*➸ Members : ${totalMem}*
...
*➸ Group Description : ${desc}*`)
            break
        case `${prefix}ptl`:
            if (!isGroupMsg) return aksa.reply(from, 'Perintah ini hanya bisa digunakan dalam group!', id)
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik !limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            aksa.sendFileFromUrl(from, pep, 'pptl.jpg', 'Follow ig : https://www.instagram.com/ptl_repost untuk mendapatkan penyegar timeline lebih banyak', message.id)
            break
        case `${prefix}koin`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (!isGroupMsg) return aksa.reply(from, 'Perintah ini hanya bisa digunakan dalam group!', id)
            ...
            if (side == 1) {
                aksa.sendStickerfromUrl(from, 'https://i.ibb.co/YTWZrZV/2003-indonesia-500-rupiah-copy.png', { method: 'get' })
            } else {
                aksa.sendStickerfromUrl(from, 'https://i.ibb.co/bLsRM2P/2003-indonesia-500-rupiah-copy-1.png', { method: 'get' })
            }
            break
        case `${prefix}dadu`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            ...
            await aksa.sendStickerfromUrl(from, 'https://www.random.org/dice/dice' + dice + '.png', { method: 'get' })
            break
        case `${prefix}spamcall`:
            ...
            await aksa.sendText(from, `Logs : ${logs}` + '.')
            break
        case `${prefix}tts`:
        case `${prefix}tss`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            ...
            await limitAdd(serial)
            if (args.length === 1) return aksa.reply(from, `Kirim perintah *!tts <kode_bahasa> <teks>*, contoh *!tts id halo semua* untuk kode bahasa silakan cek di https://bit.ly/3kAELDe `)
            var dataBhs = body.slice(5, 7)
            const ttsWK = require('node-gtts')(dataBhs)
            var dataText = body.slice(8)
            if (dataText === '') return aksa.reply(from, 'hah?', id)
            if (dataText.length > 500) return aksa.reply(from, 'Teks terlalu panjang!', id)
            ttsWK.save('./media/tts/tts.mp3', dataText, function() {
                aksa.sendPtt(from, './media/tts/tts.mp3', id)
            })
            break
        case `${prefix}family100`: //thanks to LORDZ BOT
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (!isGroupMsg) return aksa.reply(from, 'Perintah ini hanya bisa digunakan dalam group!', id)
            if (isLimit(serial)) return aksa.reply(from, `_Hai ${pushname} Limit request anda sudah mencapai batas, Akan direset kembali setiap jam 9 dan gunakan seperlunya!_`, id)
            if (isMsgLimit(serial)) {
                return
            } else {
            ...
            aksa.reply(from, `*FAMILY 100*\n\n*Pertanyaan* : ${family.result.soal}\n\n_Waktu : 30 Detik..._`, id)
            await sleep(10000)
            aksa.sendText(from, `_Sisa Waktu : 20 Detik_`)
            await sleep(10000)
            aksa.sendText(from, `_Sisa Waktu : 10 Detik_`)
            await sleep(10000)
            aksa.sendText(from, `_Waktu Habis..._`, id)
            await sleep(1000)
            aksa.reply(from, `*Jawaban* : ${family.result.jawaban}`, id)
            break
        case `${prefix}caklontong`: //thanks to LORDZ BOT
            ...
            aksa.reply(from, `*TTS CAK LONTONG*\n\n*Pertanyaan* : ${cakl.result.soal} \n\n_Waktu : 30 Detik..._`, id)
            await sleep(10000)
            aksa.sendText(from, `_Sisa Waktu : 20 Detik_`)
            await sleep(10000)
            aksa.sendText(from, `_Sisa Waktu : 10 Detik_`)
            ...
            aksa.sendText(from, `_Waktu Habis..._`, id)
            await sleep(1000)
            aksa.reply(from, cakl.result.desk, id)
            break
        case `${prefix}tebakgambar`: //thanks to LORDZ BOT
            ...
            await aksa.sendFileFromUrl(from, soalImg, 'soal.jpg', `*TEBAK GAMBAR*\n\n_Waktu : 30 Detik..._`, id)
            await sleep(10000)
            aksa.sendText(from, `_Sisa Waktu : 20 Detik_`)
            await sleep(10000)
            aksa.sendText(from, `_Sisa Waktu : 10 Detik_`)
            ...
            aksa.sendText(from, `_Waktu Habis..._`, id)
            await sleep(1000)
            aksa.reply(from, `*Jawaban* : ${teabaks.result.jawaban}`, id)
            break
        case `${prefix}artimimpi`:
            ...
            await limitAdd(serial)
            if (args.length === 1) return aksa.reply(from, 'Kirim perintah *!artimimpi <teks>*. Jika kesulitan silakan lihat contoh di *!readme*', id)
            const qmimpi = (body.slice(11))
            try {
                const mim = await axios.get(`https://api.vhtear.com/artimimpi?query=${qmimpi}&apikey=${vhtear}`)
                if (mim.data.error) return aksa.reply(from, mim.data.error, id)
                const kbbau = `➸ *Query* : ${qmimpi}\n➸ *Hasil* : ${mim.data.result.hasil}`
                aksa.reply(from, kbbau, id)
            } catch (err) {
                console.error(err.message)
                await aksa.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Maaf, tidak ditemukan')
        ...
        case `${prefix}kapankah`:
            ...
            if (!when) aksa.reply(from, `⚠️ Format salah! Ketik *${prefix}menu* untuk penggunaan.`)
            await aksa.sendText(from, `Pertanyaan: *${when}* \n\nJawaban: ${ans}`)
            break
        case `${prefix}nilai`:
        case `${prefix}rate`:
            if (!isGroupMsg) return aksa.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            ...
            if (!rating) aksa.reply(from, `⚠️ Format salah! Ketik *${prefix}menu* untuk penggunaan.`)
            await aksa.sendText(from, `Pertanyaan: *${rating}* \n\nJawaban: ${awr}`)
            break
        case `${prefix}apakah`:
            if (!isGroupMsg) return aksa.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            const nanya = args.join(' ')
            const jawab = apakah[Math.floor(Math.random() * (apakah.length))]
            if (!nanya) aksa.reply(from, `⚠️ Format salah! Ketik *${prefix}menu* untuk penggunaan.`)
            await aksa.sendText(from, `Pertanyaan: *${nanya}* \n\nJawaban: ${jawab}`)
            break
         case `${prefix}bisakah`:
            if (!isGroupMsg) return aksa.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            const bsk = args.join(' ')
            const jbsk = bisakah[Math.floor(Math.random() * (bisakah.length))]
            if (!bsk) aksa.reply(from, `⚠️ Format salah! Ketik *${prefix}menu* untuk penggunaan.`)
            await aksa.sendText(from, `Pertanyaan: *${bsk}* \n\nJawaban: ${jbsk}`)
            break
        case `${prefix}tod`: //aksara
            ...
            const gmeek = await aksa.getGroupMembersId(groupId)
                ...
                aksa.sendFileFromUrl(from, sliin, 'ddas.jpg', mmkkk, id)
                await sleep(3000)
                await aksa.sendTextWithMentions(from, lllk, id)
            break
        case `${prefix}truth`:
            if(!quotedMsg) return aksa.reply(from, 'Reply pesannya!', id)
            aksa.reply(from, `${truth()}`, id)
            break
        case `${prefix}dare`:
            if(!quotedMsg) return aksa.reply(from, 'Reply pesannya!', id)
            aksa.reply(from, `${dare()}`, id)
            break
        case `${prefix}babi`:
            const gmek = await aksa.getGroupMembersId(groupId)
            let gmik = gmek[Math.floor(Math.random() * gmek.length)]
            const mmkk = `YANG PALING BABI DISINI ADALAH @${gmik.replace(/@c.us/g, '')}`
            aksa.sendTextWithMentions(from, mmkk, id)
            break
        case `${prefix}ganteng`:
            ...
            aksa.sendTextWithMentions(from, mmkkkk, id)
            break
        case `${prefix}zodiak`:
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik #limit Untuk Mengecek Kuota Limit Kamu`, id)
            
            await limitAdd(serial)
            if (args.length === 1) return aksa.reply(from, 'Kirim perintah *#zodiak [zodiak kamu]*\nContoh : *#zodiak scorpio*', id)
            try {
            const resp = await axios.get('https://api.vhtear.com/zodiak?query=' + body.slice(8) + '&apikey=' + vhtear)
            if (resp.data.error) return aksa.reply(from, resp.data.error, id)
            ...
            aksa.reply(from, anm2, id)
            } catch (err) {
                console.error(err.message)
                await aksa.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Maaf, Zodiak tidak ditemukan')
        ...
        case `${prefix}heroml`:
            ...
            await limitAdd(serial)
            if (args.length === 1) return aksa.reply(from, 'Kirim perintah *#heroml [nama hero]*\nContoh : *#heroml akai*', id)
            try {
            ...
            aksa.sendFileFromUrl(from, resp.data.result.pictHero, 'hero.jpg', anm2, id)
            } catch (err) {
                console.error(err.message)
                await aksa.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Maaf, Hero tidak ditemukan')
        ...
        case `${prefix}indohot`:
            ...
            await limitAdd(serial)
            if (isGroupMsg) {
                if (!isNsfw) return aksa.reply(from, 'Command/Perintah NSFW belum diaktifkan di group ini!', id)
            const dsa = await get.get(`https://arugaz.herokuapp.com/api/indohot`).json()
            const { country, durasi, genre, judul, url } = await dsa.result
            await aksa.sendText(from, `*Judul* : ${judul}\n*Durasi* : ${durasi}\n*Genre* : ${genre}\n*Negara* : ${country}\n*Link* : ${url}`, id)
            }
            break
        case `${prefix}cerpen`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            ...
            aksa.reply(from, skyaaa.result, id)
            break
        case `${prefix}hilih`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik !limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            aksa.reply(from, skyaaara.result, id)
            } else {
        ...
        case `${prefix}mock`:
            ...
            aksa.reply(from, skyaaaras.result, id)
            } else {
        ...
        case `${prefix}lirik`:
            ...
            await limitAdd(serial)
            if (args.length == 1) return aksa.reply(from, 'Kirim perintah *!lirik <optional>*, contoh *!lirik aku bukan boneka*', id)
            const lagu = body.slice(7)
            const lirik = await liriklagu(lagu)
            aksa.reply(from, lirik, id)
            break
        case `${prefix}chord`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik !limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            await limitAdd(serial)
            if (args.length === 1) return aksa.reply(from, 'Kirim perintah *!chord <query>*, contoh *!chord aku bukan boneka*', id)
            const query__ = body.slice(7)
            const chord = await get.get(`https://mhankbarbars.herokuapp.com/api/chord?q=${query__}&apiKey=${apiKey}`).json()
            if (chord.error) return aksa.reply(from, chord.error, id)
            aksa.reply(from, chord.result, id)
            break
        case `${prefix}say`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (!isGroupMsg) return aksa.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            ...
            aksa.reply(from, rondKoy, id)
            break
        case `${prefix}addsay`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (args.length == 1) return aksa.reply(from, `Kirim perintah *!addsay [teks]*, contoh *!addsay anjay*`, id)
            const says = body.slice(8)
            say.push(says)
            fs.writeFileSync('./lib/say.json', JSON.stringify(say))
            aksa.reply(from, `Add ${says} sukses!`, id)
            break
        case `${prefix}delsay`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (args.length == 1) return aksa.reply(from, `Kirim perintah *!addsay [teks]*, contoh *!addsay anjay*`, id)
            ...
            fs.writeFileSync('./lib/say.json', JSON.stringify(say))
            aksa.reply(from, `Delete ${sayso} sukses!`, id)
            break
        case `${prefix}saylist`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            ...
            await aksa.reply(from, saylisto, id)
            break
        case `${prefix}quote`:
        case `${prefix}quotes`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            ...
            aksa.reply(from, `➸ *Quotes* : ${quotez2.data.quotes}\n➸ *Author* : ${quotez2.data.author}`, id)
            break
        case `${prefix}renungan`:
            ...
            try {
            const { Isi, judul, pesan } = agggg
            const rn = `➸ *Judul* : ${judul}\n➸ *Pesan* : ${pesan}\n➸ *Isi* : ${Isi}`
            aksa.reply(from, rn, id)
            } catch(err) {
                aksa.reply(from, 'Error!', id)
            }
            break
        case `${prefix}ramalpasangan`:
            ...
            await limitAdd(serial)
            if (args.length === 1) return aksa.reply(from, 'Kirim perintah *!ramalpasangan [kamu|pasangan]*\nContoh : *!ramalpasangan Lulu|Cya*', id)
            arg = body.trim().split('|')
            if (arg.length >= 2) {
                aksa.reply(from, mess.wait, id)
                const kamu = arg[0]
        ...
        case `${prefix}artinama`:
            ...
            await limitAdd(serial)
            if (args.length === 1) return aksa.reply(from, 'Kirim perintah *!artinama [query]*\nContoh : *!artinama tobz*', id)
            try {
                ...
                console.error(err.message)
                await aksa.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Maaf, User tidak ditemukan')
        ...
        case `${prefix}googleimage`:
            ...
            const qwery = argz[1]
            const jum = argz[2]
            if(!qwery) return await aksa.reply(from, `Kirim perintah *!googleimage [ |Query|Jumlah ]*, contoh = !googleimage |loli|3`, id)
            if(!jum) return await aksa.reply(from, `Jumlah gambar diperlukan, contoh = !googleimage |loli|3`, id)
            if(jum >= 5) return await aksa.reply(from, 'Jumlah terlalu banyak! Max 4', id)
        ...
        case `${prefix}meme`:
            ...
            aksa.sendFileFromUrl(from, `${url}`, 'meme.jpg', `${title}`)
            break
        case `${prefix}yourpic`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (!isGroupMsg) return aksa.reply(from, 'Fitur ini hanya bisa di gunakan dalam group', id)
            ...
            aksa.sendFileFromUrl(from, ypfp, 'pfpy.jpg', `Nih kak`)
            break
        ...
        case `${prefix}mypic`:
            ...
            aksa.sendFileFromUrl(from, mpfp, 'pfpm.jpg', `Nih kak`)
            break
        case `${prefix}ssweb`:
            if (!isGroupMsg) return aksa.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik @limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            await limitAdd(serial)
            if (args.length === 1) return aksa.reply(from, 'Kirim perintah *@ssweb [linkWeb]*\nContoh : *@ssweb https://neonime.vip*', id)
            const ssw = await axios.get('https://mhankbarbars.herokuapp.com/api/url2image?url=' + body.slice(7) + '&apiKey=' + apiKey)
            const ssww = ssw.data
            if (ssww.error) return aksa.reply(from, ssww.error, id)
            const ssw2 = `Filesize: ${ssww.filesize}`
            aksa.sendFileFromUrl(from, ssww.result, 'ssweb.jpg', ssw2, id)
            break
        case `${prefix}covid`:
            if (!isGroupMsg) return aksa.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik @limit Untuk Mengecek Kuota Limit Kamu`, id)
        ...
        case `${prefix}resepmasakan`:
            ...
            await limitAdd(serial)
            if (args.length === 1)  return aksa.reply(from, 'Kirim perintah *@resepmasakan [optional]*\nContoh *@resepmasakan rawon*', id)
            arg = body.trim().split(' ')
            console.log(...arg[1])
            var slicedArgs = Array.prototype.slice.call(arg, 1);
            console.log(slicedArgs)
...
➸ *Cara:* ${cara}`
            ...
            const base64 = `data:image/jpg;base64,${pictk.toString("base64")}`
            aksa.sendImage(from, base64, title, rmk3)
            } catch (err) {
             console.error(err.message)
             await aksa.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Maaf, Resep tidak ditemukan')
        ...
        case `${prefix}slap`: //thanks to SASHA BOT
            ...
            await aksa.sendGiphyAsSticker(from, 'https://media.giphy.com/media/S8507sBJm1598XnsgD/source.gif')
            aksa.sendTextWithMentions(from, `${prefix}` + jejiik + ' *slapped* ' + arg[1])
            break
        case `${prefix}hug`: //thanks to SASHA BOT
            arg = body.trim().split(' ')
            const janjing = author.replace('@c.us', '')
            await aksa.sendGiphyAsSticker(from, 'https://media.giphy.com/media/od5H3PmEG5EVq/giphy.gif')
            aksa.sendTextWithMentions(from, `${prefix}` + janjing + ' *peyuuuk* ' + arg[1])
            break
        case `${prefix}nye`: //thanks to SASHA BOT
            arg = body.trim().split('')
            const jancuk7 = author.replace('@c.us', '')
            await aksa.sendGiphyAsSticker(from, 'https://media.giphy.com/media/cute-baka-13LunYkkBppSBa/giphy.gif')
            aksa.sendTextWithMentions(from, `${prefix}` + jancuk7 +' *nye nye ' + arg[1])
            break
        case `${prefix}pat`: //thanks to SASHA BOT
            arg = body.trim().split(' ')
            const jartod = author.replace('@c.us', '')
            await aksa.sendGiphyAsSticker(from, 'https://media.giphy.com/media/Z7x24IHBcmV7W/giphy.gif')
            aksa.sendTextWithMentions(from, `${prefix}` + jartod + ' *👈 Si Mengelu-elus si👉* ' + arg[1])
            break
        ...
        case `${prefix}tulis`:
            ...
            await limitAdd(serial)
            if (args.length === 1) return aksa.reply(from, 'Kirim perintah *!nulis <teks>*. Jika kesulitan silakan lihat contoh di *!readme*', id)
            const nulis = encodeURIComponent(body.slice(7))
            aksa.reply(from, mess.wait, id)
            await aksa.sendFileFromUrl(from, `https://api.vhtear.com/write?text=${nulis}&apikey=${vhtear}`, 'nulis.jpg', `Nih, jangan jadi males nulis yaa ${pushname}!❤️`, id)
                .then(() => console.log('Success sending write image!'))
                .catch(async (err) => {
                    console.error(err)
                    await aksa.reply(from, 'Error!', id)
                })
            break
        case `${prefix}kbbi`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            ...
            await limitAdd(serial)
            if (args.length === 1) return aksa.reply(from, 'Kirim perintah *!kbbi <teks>*. Jika kesulitan silakan lihat contoh di *!readme*', id)
        ...
        case `${prefix}puisi1`:
            ...
            aksa.reply(from, skyaaaz.result, id)
            break
        case `${prefix}puisi2`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik !limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            aksa.reply(from, skyaaaq.result, id)
            break
        case `${prefix}puisi3`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik !limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            aksa.reply(from, skyaaar.result, id)
            break
        case `${prefix}brainly`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik !limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            await limitAdd(serial)
            if (args.length === 1) return aksa.reply(from, 'Kirim perintah *!brainly <teks>*. Jika kesulitan silakan lihat contoh di *!readme*', id)
        ...
        case `${prefix}nobadword`:
            if (!isGroupMsg) return aksa.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isGroupAdmins) return aksa.reply(from, 'Perintah ini hanya bisa di gunakan oleh Admin group!', id)
            if (args.length === 1) return aksa.reply(from, 'Pilih enable atau disable!', id)
            if (args[1].toLowerCase() === 'enable') {
                badword.push(chat.id)
                fs.writeFileSync('./lib/badword.json', JSON.stringify(badword))
                aksa.reply(from, 'Fitur Anti BadWord berhasil di aktifkan di group ini!', id)
            } else if (args[1].toLowerCase() === 'disable') {
                badword.splice(chat.id, 1)
                fs.writeFileSync('./lib/badword.json', JSON.stringify(badword))
                aksa.reply(from, 'Fitur Anti BadWord berhasil di nonaktifkan di group ini!', id)
            } else {
                aksa.reply(from, 'Pilih enable atau disable udin!', id)
            }
            break
		case `${prefix}nolinkgc`:
            ...
            if (args[1].toLowerCase() === 'enable') {
                ...
                aksa.reply(from, 'Fitur NoLink berhasil di aktifkan di group ini!', id)
            } else if (args[1].toLowerCase() === 'disable') {
                NoLink.splice(chat.id, 1)
                fs.writeFileSync('./lib/NoLink.json', JSON.stringify(NoLink))
                aksa.reply(from, 'Fitur NoLink berhasil di nonaktifkan di group ini!', id)
            } else {
                aksa.reply(from, 'Pilih enable atau disable udin!', id)
            }
            break
        case `${prefix}nsfw`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (!isGroupMsg) return aksa.reply(from, 'Perintah ini hanya bisa digunakan dalam group!', id)
            if (!isGroupAdmins) return aksa.reply(from, 'Perintah ini hanya bisa digunakan oleh Admin group!', id)
            if (args.length === 1) return aksa.reply(from, 'Pilih enable atau disable!', id)
            if (args[1].toLowerCase() === 'enable') {
                nsfw_.push(chat.id)
                fs.writeFileSync('./lib/NSFW.json', JSON.stringify(nsfw_))
                aksa.reply(from, 'NSWF Command berhasil diaktifkan di group ini! kirim perintah *!nsfwMenu* untuk mengetahui menu', id)
            } else if (args[1].toLowerCase() === 'disable') {
                nsfw_.splice(chat.id, 1)
                fs.writeFileSync('./lib/NSFW.json', JSON.stringify(nsfw_))
                aksa.reply(from, 'NSFW Command berhasil di nonaktifkan di group ini!', id)
            } else {
                aksa.reply(from, 'Pilih enable atau disable!', id)
            }
            break
        case `${prefix}welcome`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            ...
            if (args[1].toLowerCase() === 'enable') {
                ...
                aksa.reply(from, 'Fitur welcome berhasil di aktifkan di group ini!', id)
            } else if (args[1].toLowerCase() === 'disable') {
                welkom.splice(chat.id, 1)
                fs.writeFileSync('./lib/welcome.json', JSON.stringify(welkom))
                aksa.reply(from, 'Fitur welcome berhasil di nonaktifkan di group ini!', id)
            } else {
                aksa.reply(from, 'Pilih enable atau disable!', id)
            }
            break
        case `${prefix}left`:
            ...
            if (args[1].toLowerCase() === 'enable') {
                ...
                aksa.reply(from, 'Fitur left berhasil di aktifkan di group ini!', id)
            } else if (args[1].toLowerCase() === 'disable') {
                left.splice(chat.id, 1)
                fs.writeFileSync('./lib/left.json', JSON.stringify(left))
                aksa.reply(from, 'Fitur left berhasil di nonaktifkan di group ini!', id)
            } else {
                aksa.reply(from, 'Pilih enable atau disable udin!', id)
            }
            break
        case `${prefix}simi`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (!isGroupMsg) return aksa.reply(from, 'Fitur ini hanya bisa digunakan dalam group', id)
            if (!isGroupAdmins) return aksa.reply(from, 'Fitur ini hanya bisa digunakan oleh admin group', id)
            if (args.length === 1) return aksa.reply(from, 'Pilih enable atau disable!', id)
            if (args[1].toLowerCase() === 'enable') {
                simi_.push(chat.id)
                fs.writeFileSync('./lib/Simsimi.json', JSON.stringify(simi_))
                aksa.reply(from, 'Simsimi berhasil di aktifkan! Kirim perintah *[teks]*\nContoh : ! halo', id)
            } else if (args[1].toLowerCase() === 'disable') {
                simi_.splice(chat.id, 1)
                fs.writeFileSync('./lib/Simsimi.json', JSON.stringify(simi_))
                aksa.reply(from, 'Simsimi berhasil di nonaktifkan', id)
            } else {
                aksa.reply(from, 'Pilih enable atau disable!', id)
            }
            break
        case `${prefix}linkgroup`:
            if (!isBotGroupAdmins) return aksa.reply(from, 'Perintah ini hanya bisa digunakan ketika Lucya menjadi admin', id)
            if (isGroupMsg) {
                const inviteLink = await aksa.getGroupInviteLink(groupId);
                aksa.sendLinkWithAutoPreview(from, inviteLink, `\nLink group : `)
            } else {
                aksa.reply(from, 'Perintah ini hanya bisa digunakan dalam group!', id)
            }
            break
        case `${prefix}resetlinkgroup`:
            ...
            aksa.sendTextWithMentions(from, `Link group telah direset oleh admin @${sender.id.replace('@c.us', '')}`)
            }
            break
        case `${prefix}adminlist`:
            ...
            await aksa.sendTextWithMentions(from, mimin)
            break
        case `${prefix}ownergroup`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (!isGroupMsg) return aksa.reply(from, 'Perintah ini hanya bisa digunakan dalam group!', id)
            const Owner_ = chat.groupMetadata.owner
            await aksa.sendTextWithMentions(from, `Owner Group : @${Owner_}`)
            break
        case `${prefix}mentionall`:
            ...
            hehe += ''
            await aksa.sendTextWithMentions(from, hehe)
            break
        case `${prefix}kickall`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik !limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            if (!isGroupOwner) return aksa.reply(from, 'Perintah ini hanya bisa digunakan oleh Owner group', id)
            if (!isBotGroupAdmins) return aksa.reply(from, 'Perintah ini hanya bisa digunakan ketika Lucya menjadi admin', id)
            ...
            aksa.reply(from, 'Succes kick all member', id)
            break
        case `${prefix}add`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik !limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            const orang = args[1]
            if (!isGroupMsg) return aksa.reply(from, 'Fitur ini hanya bisa digunakan dalam group', id)
            if (args.length === 1) return aksa.reply(from, 'Untuk menggunakan fitur ini, kirim perintah *!add* 628xxxxx', id)
            if (!isGroupAdmins) return aksa.reply(from, 'Perintah ini hanya bisa digunakan oleh admin group', id)
            if (!isBotGroupAdmins) return aksa.reply(from, 'Perintah ini hanya bisa digunakan ketika Lucya menjadi admin', id)
            try {
                await aksa.addParticipant(from, `${orang}@c.us`)
            } catch {
                aksa.reply(from, mess.error.Ad, id)
            }
            break
        case `${prefix}kick`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            ...
            if (!isGroupAdmins) return aksa.reply(from, 'Perintah ini hanya bisa digunakan oleh admin group', id)
            if (!isBotGroupAdmins) return aksa.reply(from, 'Perintah ini hanya bisa digunakan ketika Lucya menjadi admin', id)
            if (mentionedJidList.length === 0) return aksa.reply(from, 'Untuk menggunakan Perintah ini, kirim perintah *!kick* @tagmember', id)
            await aksa.sendText(from, `Berdasar perintah ${pushname}, *Lucya* mengeluarkan:\n${mentionedJidList.join('\n')}`)
            for (let i = 0; i < mentionedJidList.length; i++) {
                if (groupAdmins.includes(mentionedJidList[i])) return aksa.reply(from, mess.error.Ki, id)
        ...
        case `${prefix}leave`:
            if (!isGroupMsg) return aksa.reply(from, 'Perintah ini hanya bisa digunakan dalam group', id)
            if (!isGroupAdmins) return aksa.reply(from, 'Perintah ini hanya bisa digunakan oleh admin group', id)
            await aksa.sendText(from, 'bye').then(() => aksa.leaveGroup(groupId))
            break
        case `${prefix}oleave`:
            if (!isGroupMsg) return aksa.reply(from, 'Perintah ini hanya bisa digunakan dalam group', id)
            if (!isAdmin) return aksa.reply(from, 'Perintah ini hanya bisa digunakan oleh admin group', id)
            await aksa.sendText(from, 'bye').then(() => aksa.leaveGroup(groupId))
            break
        case `${prefix}promote`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            ...
            if (!isGroupMsg) return aksa.reply(from, 'Fitur ini hanya bisa digunakan dalam group', id)
            if (!isGroupAdmins) return aksa.reply(from, 'Fitur ini hanya bisa digunakan oleh admin group', id)
            if (!isBotGroupAdmins) return aksa.reply(from, 'Fitur ini hanya bisa digunakan ketika Lucya menjadi admin', id)
            if (mentionedJidList.length === 0) return aksa.reply(from, 'Untuk menggunakan fitur ini, kirim perintah *!promote* @tagmember', id)
            if (mentionedJidList.length >= 2) return aksa.reply(from, 'Maaf, perintah ini hanya dapat digunakan kepada 1 user.', id)
            if (groupAdmins.includes(mentionedJidList[0])) return aksa.reply(from, 'Maaf, user tersebut sudah menjadi admin.', id)
            await aksa.promoteParticipant(groupId, mentionedJidList[0])
            await aksa.sendTextWithMentions(from, `Perintah diterima, menambahkan @${mentionedJidList[0]} sebagai admin.`)
            break
        case `${prefix}demote`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik !limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            if (!isGroupAdmins) return aksa.reply(from, 'Fitur ini hanya bisa digunakan oleh admin group', id)
            if (!isBotGroupAdmins) return aksa.reply(from, 'Fitur ini hanya bisa digunakan ketika Lucya menjadi admin', id)
            if (mentionedJidList.length === 0) return aksa.reply(from, 'Untuk menggunakan fitur ini, kirim perintah *!demote* @tagadmin', id)
            if (mentionedJidList.length >= 2) return aksa.reply(from, 'Maaf, perintah ini hanya dapat digunakan kepada 1 orang.', id)
            if (!groupAdmins.includes(mentionedJidList[0])) return aksa.reply(from, 'Maaf, user tersebut tidak menjadi admin.', id)
            await aksa.demoteParticipant(groupId, mentionedJidList[0])
            await aksa.sendTextWithMentions(from, `Perintah diterima, menghapus jabatan @${mentionedJidList[0]}.`)
            break
        case `${prefix}delete`:
        case `${prefix}del`:
            if (!isGroupMsg) return aksa.reply(from, 'Fitur ini hanya bisa digunakan dalam group', id)
            if (!isGroupAdmins) return aksa.reply(from, 'Fitur ini hanya bisa digunakan oleh admin group', id)
            if (!quotedMsg) return aksa.reply(from, 'Salah!!, kirim perintah *!delete <tagpesanbot>*', id)
            if (!quotedMsgObj.fromMe) return aksa.reply(from, 'Salah!, Bot tidak bisa mengahpus chat user lain!', id)
            aksa.deleteMessage(quotedMsgObj.chatId, quotedMsgObj.id, false)
            break
        case `${prefix}odel`:
            if (!isGroupMsg) return aksa.reply(from, 'Fitur ini hanya bisa digunakan dalam group', id)
            if (!isAdmin) return aksa.reply(from, 'Fitur ini hanya bisa digunakan oleh admin Lucya', id)
            if (!quotedMsg) return aksa.reply(from, 'Salah!!, kirim perintah *!delete <tagpesanbot>*', id)
            if (!quotedMsgObj.fromMe) return aksa.reply(from, 'Salah!, Bot tidak bisa mengahpus chat user lain!', id)
            aksa.deleteMessage(quotedMsgObj.chatId, quotedMsgObj.id, false)
            break
        case `${prefix}sider`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if(!isGroupMsg) return aksa.reply(from, 'Hanya untuk di dalam group')
            if(!quotedMsg) return aksa.reply(from,'Tolong reply pesan')
            if(!quotedMsgObj) return aksa.reply(from,'Tolong reply pesan')
            try{
                ...
                aksa.sendTextWithMentions(from, `hmm read doang..\n${lista}`)
            } catch(err) {
                console.log(err)
                aksa.reply(from,'Maaf, Belum ada yang membaca pesan')
            }     
            break
        case `${prefix}closegc`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (!isGroupMsg) return aksa.reply(from, 'Perintah ini hanya bisa digunakan dalam group!', id)
            if (!isGroupAdmins) return aksa.reply(from, 'Perintah ini hanya untuk Admin Grup!', id)
			...
			if(isGroupMsg) {
				if(isGroupAdmins) {
					try {
						...
						await aksa.setGroupDescription(from, `${desk}`, id)
					} catch {
						aksa.reply(from, 'Terjadi kesalahan, tidak dapat mengubah deskripsi grup', message)
					}
				}else{
					aksa.reply(from, 'Maaf, fitur ini hanya untuk owner grup', message)
				}
			}else{
				aksa.reply(from, 'Fitur ini hanya bisa di gunakan dalam grup', message)
			}
                    break
        ...
        case `${prefix}downloadmanga`:
            ...
            await limitAdd(serial)
            if (args.length === 1) return aksa.reply(from, `Kirim perintah *!downloadmanga [query]*\nContoh : *!downloadmanga darling in the franxx*`, id)
            const animep = await axios.get('https://mhankbarbars.herokuapp.com/api/komiku?q=' + body.slice(15) + '&apiKey=' + apiKey)
            if (animep.data.error) return aksa.reply(from, animep.data.error, id)
            const res_animep = `${animep.data.info}\n\n${animep.data.sinopsis}\n\n${animep.data.link_dl}`
            aksa.sendFileFromUrl(from, animep.data.thumb, 'komiku.jpg', res_animep, id)
            break
        case `${prefix}nhinfo`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik !limit Untuk Mengecek Kuota Limit Kamu`, id)
        ...
        case `${prefix}nhview`:
            ...
            if (isGroupMsg) {
                if (!isNsfw) return aksa.reply(from, 'Command/Perintah NSFW belum diaktifkan di group ini!', id)
                if (args.length === 1) return aksa.reply(from, 'Kirim perintah *!nhview [212121]*\nContoh : *!nhview 321421*', id)
                ...
                for (let i = 0; i < nhsh2.length; i++) {
                    await aksa.sendImage(from, nhsh2[i].data, 'thumbserc.jpg', '', id)
                }
            }
            break
        case `${prefix}kpop`:
            ...
            await limitAdd(serial)
            if (args.length === 1) return aksa.reply(from, `Untuk menggunakan !kpop\nSilahkan ketik: !kpop [query]\nContoh: !kpop bts\n\nquery yang tersedia:\nblackpink, exo, bts`, id)
            if (args[1] === 'blackpink' || args[1] === 'exo' || args[1] === 'bts') {
                fetch('https://raw.githubusercontent.com/tobzZ/grabbed-results/main/random/kpop/' + args[1] + '.txt')
                    ...
                    .then(body => {
                        ...
                        aksa.sendFileFromUrl(from, randomkpopx, '', 'nih..', id)
                    })
                    .catch(() => {
                        aksa.reply(from, 'Ada yang eror!', id)
                    })
            } else {
                aksa.reply(from, `Maaf query tidak tersedia. Silahkan ketik !kpop untuk melihat list query`)
            }
            break
        case `${prefix}wait`:
            if (!isGroupMsg) return aksa.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            ...
            if (isMedia && type === 'image' || quotedMsg && quotedMsg.type === 'image') {
                ...
                .then(resolt => {
                    if (resolt.docs && resolt.docs.length <= 0) {
                        aksa.reply(from, 'Maaf, saya tidak tau ini anime apa', id)
                    }
                    ...
                    aksa.sendFileFromUrl(from, video, 'nimek.mp4', teks, id).catch(() => {
                        aksa.reply(from, teks, id)
                    })
                })
                .catch(() => {
                    aksa.reply(from, 'Error !', id)
                ...
                aksa.sendFileFromUrl(from, tutor, 'Tutor.jpg', 'Neh contoh mhank!', id)
            }
            break
        case `${prefix}nsfwmenu`:
            if (!isNsfw) return
            aksa.reply(from,nsfwmenu(), id)
            break
        case `${prefix}anime`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik !limit Untuk Mengecek Kuota Limit Kamu`, id)
                    ...
                    .then((result) => {
                        rugapoi.getVideo(result.link)
                            .then((res) => {
                                ...
                                aksa.reply(from, `Title: ${res.title}\n\nLink:\n${heheq}\nmasih tester bntr :v`)
                            })
                    })
                    .catch(() => {
                        aksa.reply(from, 'Ada yang Error!', id)
                    })
            }
            break 
        case `${prefix}otakudesu`:
            ...
            await limitAdd(serial)
            if (args.length === 1) return aksa.reply(from, 'Kirim perintah *@otakudesu [query]*\nContoh : *@otakudesu darling in the franxx*', id)
            const animes = await axios.get('https://mhankbarbars.herokuapp.com/api/otakudesu?q=' + body.slice(7) + '&apiKey=' + apiKey)
            if (animes.data.error) return aksa.reply(from, animes.data.error, id)
            const res_animes = `${animes.data.title}\n\n${animes.data.info}\n\n${animes.data.sinopsis}`
            aksa.sendFileFromUrl(from, animes.data.thumb, 'otakudesu.jpg', res_animes, id)
            break
        case `${prefix}kusonime`:
            ...
            await limitAdd(serial)
            if (args.length === 1) return aksa.reply(from, 'Kirim perintah *@kusonime [query]*\nContoh : *@kusonime darling in the franxx*', id)
            const animeq = await axios.get('https://mhankbarbars.herokuapp.com/api/kuso?q=' + body.slice(7) + '&apiKey=' + apiKey)
            if (animeq.data.error) return aksa.reply(from, animeq.data.error, id)
            const res_animeq = `${animeq.data.title}\n\n${animeq.data.info}\n\n${animeq.data.sinopsis}\n\n${animeq.data.link_dl}`
            aksa.sendFileFromUrl(from, animeq.data.thumb, 'kusonime.jpg', res_animeq, id)
            break
        case `${prefix}dewabatch`:
            ...
            await limitAdd(serial)
            if (args.length === 1) return aksa.reply(from, 'Kirim perintah *@dewabatch [query]*\nContoh : *@dewabatch darling in the franxx*', id)
            const animek = await axios.get('https://mhankbarbars.herokuapp.com/api/dewabatch?q=' + body.slice(7) + '&apiKey=' + apiKey)
            if (animek.data.error) return aksa.reply(from, animek.data.error, id)
            const res_animek = `${animek.data.result}\n\n${animek.data.sinopsis}`
            aksa.sendFileFromUrl(from, animek.data.thumb, 'dewabatch.jpg', res_animek, id)
            break
        case `${prefix}komiku`:
            if (!isGroupMsg) return aksa.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik @limit Untuk Mengecek Kuota Limit Kamu`, id)
        ...
        case `${prefix}loli`:
            ...
            aksa.sendFileFromUrl(from, loly.result, 'loli.jpeg', '*LOLI*', id)
            break
        case `${prefix}shota`:
            if (!isGroupMsg) return aksa.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik @limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            aksa.sendFile(from, img, "shota.jpg", `*SHOTA*`, id)
                    }) 
                .catch(
                    (error) => {
                        console.log(error);
        ...
        case `${prefix}waifu`:
            ...
            aksa.sendFileFromUrl(from, waifu.data.image, 'Waifu.jpg', `➸ Name : ${waifu.data.name}\n➸ Description : ${waifu.data.desc}\n\n➸ Source : ${waifu.data.source}`, id)
            break
        case `${prefix}husbu`:
            if (!isGroupMsg) return aksa.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik @limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            aksa.sendFileFromUrl(from, rindKiy.image, 'Husbu.jpg', rindKiy.teks, id)
            break
        case `${prefix}randomnekonime`:
            ...
            aksa.sendFileFromUrl(from, nekon.result, `Nekonime${ext}`, 'Nekonime!', id)
            break
        case `${prefix}randomtrapnime`:
            if (!isGroupMsg) return aksa.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isNsfw) return aksa.reply(from, 'command/Perintah NSFW belum di aktifkan di group ini!', id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik @limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            aksa.sendImage(from, trapn.result, `trapnime${ext}`, 'Trapnime!', id)
            break
        case `${prefix}randomhentai`:
            ...
            aksa.sendImage(from, henta.result, `RandomHentai${ext}`, 'Random Hentai!', id)
            break
        case `${prefix}randomnsfwneko`:
            ...
            aksa.sendImage(from, nsfwn.result, `NsfwNeko${ext}`, 'NsfwNeko!', id)
            break
        case `${prefix}randomanime`:
            ...
            aksa.sendFileFromUrl(from, ranime.url, `RandomAnime${ext}`, 'Random Anime!', id)
            break
        case `${prefix}nhder`:
            if (!isGroupMsg) return aksa.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isNsfw) return aksa.reply(from, 'command/Perintah NSFW belum di aktifkan di group ini!', id)
            ...
            if (args.length >=2){
                ...
                aksa.sendText(from, caption)
            } else {
                aksa.sendText(from, 'Maaf tolong masukan code nuclear')
            }
            break
        case `${prefix}wallanime` :
            ...
            aksa.sendFileFromUrl(from, walnimek, 'Nimek.jpg', '', id)
            break
        case `${prefix}quotesnime`:
            ...
            skya_ = skya.data
            aksa.reply(from, `➸ *Quotes* : ${skya_.quote}\n➸ *Character* : ${skya_.character}\n➸ *Anime* : ${skya_.anime}`, id)
            break
        ...
        case `${prefix}malanime`:
           ...
           `https://api.jikan.moe/v3/search/anime?q=${keyword}`
            ...
            if (!parsed) {
              await aksa.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Maaf, Anime tidak ditemukan', id)
              return null
              }
...
🌐️ *URL*: ${url}`
            ...
            aksa.sendImage(from, base64, title, content)
           } catch (err) {
             console.error(err.message)
             await aksa.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Maaf, Anime tidak ditemukan')
           }
          break
...
🌐️ *URL*: ${url}`
            ...
            const image = await bent("buffer")(image_url)
            const base64 = `data:image/jpg;base64,${image.toString("base64")}`
            aksa.sendImage(from, base64, name, contentt)
           } catch (err) {
             console.error(err.message)
             await aksa.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Maaf, Anime tidak ditemukan')
           }
        ...
        case `${prefix}send`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (isGroupMsg) return aksa.reply (from, 'Perintah ini hanya bisa dilakukan di chat personal!', id)
            var cek = pengirim.includes(sender.id);
            if(!cek){
                return aksa.reply(from, 'kamu belum terdaftar, untuk mendaftar kirim !daftar no wa kamu\ncontoh : !daftar 6281281817375 ', id) //if user is not registered
            } else {           
                if (isMedia && args.length >= 1) {
                    ...
                    await aksa.reply(from, 'Format salah! Untuk membuka daftar perintah kirim !help', id)
                } 
            } 
        ...
        case `${prefix}daftar`: { //menambahkan nomor ke database 
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (isGroupMsg) return aksa.reply (from, 'Perintah ini hanya bisa dilakukan di chat personal!', id)
                if (args.length === 1) return aksa.reply(from, 'Nomornya mana kak?\ncontoh: !daftar 6281281817375')  
                const text = body.slice(8).replace(/[-\s+]/g,'') + '@c.us'
                var cek = pengirim.includes(text);
                if(cek){
                    return aksa.reply(from, 'Nomor sudah ada di database', id) //if number already exists on database
                } else {
                    ...
                    if (!hasill) return aksa.reply(from, 'Nomor WhatsApp tidak valid [ Tidak terdaftar di WhatsApp ] atau gunakan 62 di awal bukan 0', id) 
                    {
                    pengirim.push(mentah.id._serialized)
                    fs.writeFileSync('./lib/user.json', JSON.stringify(pengirim))
                        aksa.sendText(from, hasill)
                    }
                }
            }
            break                                     
        ...
        if (!isOwner) return aksa.reply(from, 'Fitur ini hanya dapat digunakan oleh Owner Lucya') 
                ...
                await aksa.reply(from, listno, id)
                break
        ...
        case `${prefix}jadwalsholat`:
            if (!isGroupMsg) return aksa.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik @limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return aksa.reply(from, `[❗] Kirim perintah *@jadwalShalat [ Daerah ]*\ncontoh : *@jadwalShalat Tangerang*\nUntuk list daerah kirim perintah *@listDaerah*`)
            const daerah = body.slice(14)
            const jadwalShalat = await axios.get(`https://api.vhtear.com/jadwalsholat?query=${daerah}&apiKey=${vhtear}`)
            if (jadwalShalat.data.error) return aksa.reply(from, jadwalShalat.data.error, id)
        ...
        case `${prefix}quran`:
            if (!isGroupMsg) return aksa.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)
			if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik @limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return aksa.reply(from, `Kirim perintah Surah Quran kamu dengan cara ketik perintah :\n*@quran* [ Urutan Surat ]\nContoh :\n*@quran 1*`, id)
            ...
            await aksa.reply(from, `${hasqu}`, id).catch((e) => aksa.reply(from, `*Terdapat kesalahan saat mencari surat ${args[1]}*`, id))
            await limitAdd(serial)
            break
        case `${prefix}listsurah`:
			...
			try {
                ...
                .then((response) => {
                    ...
                    aksa.reply(from, hehex, id)
                })
            } catch(err) {
                aksa.reply(from, err, id)
            }
            break
        case `${prefix}infosurah`:
            if (!isGroupMsg) return aksa.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)
			if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik @limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length == 1) return aksa.reply(from, `Kirim perintah *@infosurah [ Nama Surah ]*\nContoh : *@infosurah al-fatihah*`, message.id)
                ...
                try {
                    ...
                    pesan += '\n\n___________________________'
                    aksa.reply(from, pesan, message.id)
                    limitAdd(serial)
                }catch{
                    aksa.reply(from, 'Data tidak ditemukan, atau nama surah salah', id)
                }
            break
        case `${prefix}tafsir`:
            if (!isGroupMsg) return aksa.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik @limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length == 1) return aksa.reply(from, `Kirim perintah *@tafsir [ Nama Surah ] [ Ayat ]*\nContoh : *@tafsir al-fatihah 2*`, message.id)
            ...
            try{
                ...
                aksa.reply(from, 'Data tidak ditemukan, mungkin nama surah/ayat salah', id)
            }
            break
        ...
        case `${prefix}ytmp4`:
            ...
            if (!isLin) return aksa.reply(from, mess.error.Iv, id)
            try {
                aksa.reply(from, mess.wait, id)
                ...
                const ytvh2 = await ytvh.json()
                 if (ytvh2.status == false) {
                    ...
                    if (Number(ytvh2.result.size.split(' MB')[0]) > 75.00) return aksa.reply(from, `Maaf durasi video sudah melebihi batas maksimal 30 MB!`, id)
                    const { title, UrlVideo, imgUrl, size } = await ytvh2.result
                    aksa.sendFileFromUrl(from, imgUrl, 'thumb.jpg', `*「 YOUTUBE MP4 」*\n\n➸ *Judul* : ${title}\n➸ *Filesize* : ${size}\n\n_*Untuk durasi lebih dari batas disajikan dalam bentuk link*._\n${UrlVideo}`, id)
                    await aksa.sendFileFromUrl(from, UrlVideo, `${title}.mp4`, '', id).catch(() => aksa.reply(from, mess.error.Yt4, id))
                    await limitAdd(serial)
                }
            } catch (err) {
        ...
        case `${prefix}ytmp3`:
            ...
            if (!isLinks) return aksa.reply(from, mess.error.Iv, id)
            try {
                aksa.reply(from, mess.wait, id)
                ...
                const vhtearyt33 = await vhtearyt3.json()
                 if (vhtearyt33.status == false) {
                    ...
                    await aksa.sendFileFromUrl(from, UrlMp3, `${title}.mp3`, '', id).catch(() => aksa.reply(from, mess.error.Yt4, id))
                    await limitAdd(serial)
                }
            } catch (err) {
        ...
        case `${prefix}smule`:
            ...
            await limitAdd(serial)
            if (args.length === 1) return aksa.reply(from, 'Kirim perintah *@smule [linkSmule]*\nContoh : *@smule https://www.smule.com/p/767512225_3062360163*', id)
            aksa.reply(from, mess.wait, id)
            arg = body.trim().split(' ')
            console.log(...arg[1])
            var slicedArgs = Array.prototype.slice.call(arg, 1);
...
➸ *Type:* ${Type}`
            ...
            aksa.sendImage(from, image, `${title}.jpg`, sml3)
            aksa.sendFileFromUrl(from, url, `${title}.mp3`, sml3, id)
            } catch (err) {
             console.error(err.message)
             await aksa.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Maaf, Music tidak ditemukan')
        ...
        case `${prefix}play`:
            ...
            if (!isGroupMsg) return aksa.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik #ceklimit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length == 1) return aksa.reply(from, `Untuk mencari lagu from youtube\n\nPenggunaan: !play judul lagu`, id)
            try {
                aksa.reply(from, mess.wait, id)
                ...
                const webplay2 = await webplay.json()
                 if (webplay2.status == false) {
                    ...
                    if (Number(webplay2.result.size.split(' MB')[0]) >= 10.00) return aksa.reply(from, 'Maaf durasi music sudah melebihi batas maksimal 10 MB!', id)
                    const { image, mp3, size, ext, title, duration } = await webplay2.result
                    const captplay = `*「 PLAY 」*\n\n➸ *Judul* : ${title}\n➸ *Durasi* : ${duration}\n➸ *Filesize* : ${size}\n➸ *Exp* : ${ext}\n\n_*Music Sedang Dikirim*_`
                    aksa.sendFileFromUrl(from, image, `thumb.jpg`, captplay, id)
                    await aksa.sendFileFromUrl(from, mp3, `${title}.mp3`, '', id).catch(() => aksa.reply(from, mess.error.Yt4, id))
                    await limitAdd(serial)
                }
                } catch (err) {
		...
		case `${prefix}facebook`:
            ...
            await limitAdd(serial)
            if (isMedialimit(serial)) return aksa.reply(from, `Maaf ${pushname}, kuota limit media kamu sudah habis, ketik !limed untuk mengecek kuota Limit media kamu`, id)
            await MedialimitAdd(serial)
			if (args.length === 1) return aksa.reply(from, `Untuk mendownload video from link facebook\nketik: !fb [link_fb]`, id)
            if (!args[1].match(isUrl) && !args[1].includes('facebook.com')) return aksa.reply(from, `Maaf, link yang kamu kirim tidak valid. [Invalid Link]`, id)
            await aksa.reply(from, mess.wait, id);
            try {
                    aksa.reply(from, mess.wait, id)
                    const respi = await axios.get('https://api.vhtear.com/fbdl?link=' + args[1] + '&apikey=' + vhtear)
                    const { VideoUrl } = respi.data.result
                    aksa.sendFileFromUrl(from, VideoUrl, `fb.mp4`, '', id)
                } catch (err) {
                    console.error(err.message)
                    await aksa.sendFileFromUrl(from, errorurl, 'error.png', '💔️ Maaf, Video tidak ditemukan')
        ...
        case `${prefix}twitter`:
            ...
            if (isMedialimit(serial)) return aksa.reply(from, `Maaf ${pushname}, kuota limit media kamu sudah habis, ketik !limed untuk mengecek kuota Limit media kamu`, id)
            await MedialimitAdd(serial)
            if (args.length === 1) return aksa.reply(from, 'Kirim perintah *!twitter [linkTwitter]* untuk contoh silahkan kirim perintah *!readme*')
            arg = body.trim().split(' ')
            console.log(...arg[1])
            var slicedArgs = Array.prototype.slice.call(arg, 1);
            console.log(slicedArgs)
...
• *Deskripsi:* ${desk}`
            ...
            aksa.sendFileFromUrl(from, urlVideo, `Twitter.mp4`, twtdl3, id)
            await limitAdd(serial)
            } catch (err) {
             console.error(err.message)
             await aksa.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Maaf, Video tidak ditemukan')
           }
          break
        case `${prefix}ig`: 
        case `${prefix}instagram`:
            ...
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik !limit Untuk Mengecek Kuota Limit Kamu`, id)
                ...
                if (isMedialimit(serial)) return aksa.reply(from, `Maaf ${pushname}, kuota limit media kamu sudah habis, ketik !limed untuk mengecek kuota Limit media kamu`, id)
            await MedialimitAdd(serial)
            if (args.length === 1) return aksa.reply(from, `Kirim perintah *!ig [ Link Instagram ]* untuk contoh silahkan kirim perintah *!readme*`)
            if (!args[1].match(isUrl) && !args[1].includes('instagram.com')) return aksa.reply(from, `Maaf, link yang kamu kirim tidak valid. [Invalid Link]`, id)
            await aksa.reply(from, mess.wait, id);
            instagram(args[1]).then(async(res) => {
                ...
                if (res.post[i].type == "image") {
                        await aksa.sendFileFromUrl(from, res.post[i].urlDownload, "ig.jpg", `*「 INSTAGRAM 」*\n\n➸ *Username* : ${username}\n➸ *Tipe* : Image/Jpg`, id);
                        limitAdd(serial)
                    } else if (res.post[i].type == "video") {
                        await aksa.sendFileFromUrl(from, res.post[i].urlDownload, "ig.mp4", `*「 INSTAGRAM 」*\n\n➸ *Username* : ${username}\n➸ *Tipe* : Video/MP4`);
                        limitAdd(serial)
                    }
                }
            }).catch((err) => {
                console.log(err);
                aksa.reply(from, `Maaf, Terjadi Kesalahan`, id)
            })
            break
        case `${prefix}igstory`:
     //premium command, lu tau kemana harus ngehubungi
        ...
        case `${prefix}xnxx`:
            ...
            if (isMedialimit(serial)) return aksa.reply(from, `Maaf ${pushname}, kuota limit media kamu sudah habis, ketik !limed untuk mengecek kuota Limit media kamu`, id)
            await MedialimitAdd(serial)
            if (args.length === 1) return aksa.reply(from, 'Kirim perintah *!xnxx [linkXnxx]*, untuk contoh silahkan kirim perintah *!readme*')
            if (!args[1].match(isUrl) && !args[1].includes('xnxx.com')) return aksa.reply(from, mess.error.Iv, id)
            try {
                aksa.reply(from, mess.wait, id)
                const resq = await axios.get('https://mhankbarbars.herokuapp.com/api/xnxx?url=' + args[1] + '&apiKey=' + apiKey)
                const resp = resq.data
                if (resp.error) {
                    aksa.reply(from, ytvv.error, id)
                } else {
                    if (Number(resp.result.size.split(' MB')[0]) > 20.00) return aksa.reply(from, 'Maaf durasi video sudah melebihi batas maksimal 20 menit!', id)
                    aksa.sendFileFromUrl(from, resp.result.thumb, 'thumb.jpg', `➸ *Judul* : ${resp.result.judul}\n➸ *Deskripsi* : ${resp.result.desc}\n➸ *Filesize* : ${resp.result.size}\n\nSilahkan tunggu sebentar proses pengiriman file membutuhkan waktu beberapa menit.`, id)
                    await aksa.sendFileFromUrl(from, resp.result.vid, `${resp.result.title}.mp4`, '', id)
                }
            } catch (err) {
                console.log(err)
                await aksa.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Maaf, Video tidak ditemukan')
        ...
        case `${prefix}images`:
                ...
                await limitAdd(serial)
                if (args.length === 1) return aksa.reply(from, `Untuk mencari gambar di pinterest\nketik: !images [search]\ncontoh: !images naruto`, id)
                const cariwall = body.slice(8)
                const hasilwall = await images.fdci(cariwall)
                aksa.sendFileFromUrl(from, hasilwall, '', '', id)
                    .catch(() => {
                        aksa.reply(from, 'Ada yang eror!', id)
                    })
                break
        ...
        case `${prefix}smulestalk`:
            ...
            await limitAdd(serial)
            if (args.length === 1) return aksa.reply(from, 'Kirim perintah *@smulestalk [@username]*\nContoh : *@smulestalk loli*', id)
            arg = body.trim().split(' ')
            console.log(...arg[1])
            var slicedArgs = Array.prototype.slice.call(arg, 1);
            console.log(slicedArgs)
...
➸ *Total Rekaman:* ${recording}`
            ...
            const base64 = `data:image/jpg;base64,${pictk.toString("base64")}`
            aksa.sendImage(from, base64, title, smule)
            } catch (err) {
             console.error(err.message)
             await aksa.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Maaf, User tidak ditemukan')
        ...
        case `${prefix}starmaker`:
            ...
            await limitAdd(serial)
            if (args.length === 1) return aksa.reply(from, 'Kirim perintah *@starmaker [linkStarmaker]* untuk contoh silahkan kirim perintah *@readme*')
            arg = body.trim().split(' ')
            console.log(...arg[1])
            var slicedArgs = Array.prototype.slice.call(arg, 1);
            console.log(slicedArgs)
...
➸ *Deskripsi:* ${desc}`
            ...
            const pictk = await bent("buffer")(image)
            const base64 = `data:image/jpg;base64,${pictk.toString("base64")}`
            aksa.sendImage(from, base64, 'image.jpg', 'nihh mhank')
            aksa.sendFileFromUrl(from, url, `${title}.mp4`, '', id)
            } catch (err) {
             console.error(err.message)
             await aksa.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Maaf, User tidak ditemukan')
        ...
        case `${prefix}joox`:
            ...
            if (isMedialimit(serial)) return aksa.reply(from, `Maaf ${pushname}, kuota limit media kamu sudah habis, ketik !limed untuk mengecek kuota Limit media kamu`, id)
            await MedialimitAdd(serial)
                if (args.length === 1) return aksa.reply(from, 'Kirim perintah *!joox [optional]*\nContoh : *!joox Alan Walker*', id)
                aksa.reply(from, mess.wait, id)
                arg = body.trim().split(' ')
                console.log(...arg[1])
                var slicedArgs = Array.prototype.slice.call(arg, 1);
                ...
                try {
                    ...
                    const { penyanyi, judul, album, linkImg, linkMp3, filesize, ext, duration, lirik } = music2.data.result[0]
...
➸ *Lirik: *${lirik}`
                    ...
                    const base64 = `data:image/jpg;base64,${pictk.toString("base64")}`
                    aksa.sendImage(from, base64, judul, musik)
                    aksa.sendFile(from, linkMp3, `${judul}.mp3`, '', id)
                } catch (err) {
                    console.error(err.message)
                    await aksa.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Maaf, User tidak ditemukan')
        ...
        case `${prefix}tiktok`:
            ...
            if (isMedialimit(serial)) return aksa.reply(from, `Maaf ${pushname}, kuota limit media kamu sudah habis, ketik !limed untuk mengecek kuota Limit media kamu`, id)
            await MedialimitAdd(serial)
            if (args.length === 1) return aksa.reply(from, 'Kirim perintah *!tiktok [linkTiktok]*\nContoh : *!tiktok https://vt.tiktok.com/yqyjPX/*', id)
            try {
                aksa.reply(from, mess.wait, id)
                const resp = await axios.get('https://api.vhtear.com/tiktokdl?link=' + body.slice(8) + '&apikey=' + vhtear)
                const { video, title, duration, image, desk, dibuat } = resp.data.result
                const capss = `*Video Ditemukan!*
➸ Judul : ${title}
➸ Deskripsi : ${desk}
➸ Durasi : ${duration}
➸ Dibuat : ${dibuat}`
                    aksa.sendFileFromUrl(from, video, `tiktok.mp4`, capss, id)
                } catch (err) {
                    console.error(err.message)
                    await aksa.sendFileFromUrl(from, errorurl, 'error.png', '💔️ Maaf, Video tidak ditemukan')
        ...
        case `${prefix}igstalk`:
            ...
            await limitAdd(serial)
            if (args.length === 1) return aksa.reply(from, 'Kirim perintah *!igStalk @username*\nContoh *!igStalk @bdrsmsdn*', id)
            try {
            const stalk = await get.get(`https://api.vhtear.com/igprofile?query=${args[1]}&apikey=${vhtear}`).json()
            if (stalk.error) return aksa.reply(from, stalk.error, id)
            const { username,full_name,follower,follow,biography,is_private,picture,post_count } = stalk.result
            const caps = `「 IGSTALK 」\n\n➸ *Nama* : ${full_name}\n➸ *Username* : ${username}\n➸ *Jumlah Followers* : ${follower}\n➸ *Jumlah Following* : ${follow}\n➸ *Jumlah Postingan* : ${post_count}\n➸ *Biodata* : ${biography}\n➸ *Private* : ${is_private}\n\n-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-\n\n${donate()}`
            await aksa.sendFileFromUrl(from, picture, 'Profile.jpg', caps, id)
            } catch (err) {
                console.error(err.message)
        ...
        case `${prefix}twstalk`:
            ...
            await limitAdd(serial)
            if (args.length === 1) return aksa.reply(from, 'Kirim perintah *!twstalk @username*\nContoh *!twstalk @bdrsmsdn*', id)
            const twstalk = await get.get(`https://mhankbarbars.herokuapp.com/api/twstalk?username=${args[1]}&apiKey=${apiKey}`).json()
            if (twstalk.error) return aksa.reply(from, twstalk.error, id)
            const { idd, followers_count, name, full_name, status_count, profile_pic } = twstalk
            const twcaps = `➸ *ID* : ${idd}\n➸*Nama Lengkap* : ${full_name}\n➸ *Nama* : ${name}\n➸ *Jumlah Followers* : ${followers_count}\n➸ *Jumlah Tweet* : ${status_count}\n\n-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-\n\n${donate()}`
            await aksa.sendFileFromUrl(from, profile_pic, 'Profile.jpg', twcaps, id)
            break
        ...
        case `${prefix}cekresi`:
            ...
            await limitAdd(serial)
            if (args.length === 1) return aksa.reply(from, `Maaf, format pesan salah.\nSilahkan ketik pesan dengan !resi <kurir> <no_resi>\n\nKurir yang tersedia:\njne, pos, tiki, wahana, jnt, rpx, sap, sicepat, pcp, jet, dse, first, ninja, lion, idl, rex`, id)
            const kurirs = ['jne', 'pos', 'tiki', 'wahana', 'jnt', 'rpx', 'sap', 'sicepat', 'pcp', 'jet', 'dse', 'first', 'ninja', 'lion', 'idl', 'rex']
            if (!kurirs.includes(args[1])) return aksa.sendText(from, `Maaf, jenis ekspedisi pengiriman tidak didukung layanan ini hanya mendukung ekspedisi pengiriman ${kurirs.join(', ')} Tolong periksa kembali.`)
            console.log('Memeriksa No Resi', args[2], 'dengan ekspedisi', args[1])
            cekResi(args[1], args[2]).then((result) => aksa.sendText(from, result))
            break
        case `${prefix}shortlink`:
            ...
            await limitAdd(serial)
            if (args.length === 1) return aksa.reply(from, `ketik !shortlink <url>`, id)
            if (!args[1].match(isUrl)) return aksa.reply(from, 'Maaf, url yang kamu kirim tidak valid.', id)
            const shortlink = await urlShortener(args[1])
            await aksa.sendText(from, shortlink)
                .catch(() => {
                    aksa.reply(from, 'Ada yang eror!', id)
                })
            break
        case `${prefix}url2img`:
            ...
            if (!_query.match(isUrl)) return aksa.reply(from, mess.error.Iv, id)
            if (args.length === 1) return aksa.reply(from, 'Kirim perintah *!url2img <web>*\nContoh *!url2img https://google.com*', id)
            const url2img = await get.get(`https://mhankbarbars.herokuapp.com/api/url2image?url=${_query}&apiKey=${apiKey}`).json()
            if (url2img.error) return aksa.reply(from, url2img.error, id)
            aksa.sendFileFromUrl(from, url2img.result, 'kyaa.jpg', null, id)
            break
        case `${prefix}maps`:
            if (!isGroupMsg) return aksa.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (isLimit(serial)) return aksa.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik @limit Untuk Mengecek Kuota Limit Kamu`, id)
            ...
            await limitAdd(serial)
            if (args.length === 1) return aksa.reply(from, 'Kirim perintah *@maps [optional]*, Contoh : *@maps Jakarta*')
            arg = body.trim().split(' ')
            console.log(...arg[1])
            var slicedArgs = Array.prototype.slice.call(arg, 1);
            console.log(slicedArgs)
            ...
            const base64 = `data:image/jpg;base64,${pictk.toString("base64")}`
            aksa.sendImage(from, base64, 'maps.jpg', `*Hasil Maps : ${mapz}*`)
            } catch (err) {
             console.error(err.message)
             await aksa.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Maaf, User tidak ditemukan')
        ...
        case `${prefix}translate`:
            ...
            if (args.length >= 2) {
                ...
                translatte(text, { to: codelang }).then(res => {
                    aksa.sendText(from, res.text);
                }).catch(err => {
                    aksa.sendText(from, `[ERROR] Teks tidak ada, atau kode bahasa ${codelang} tidak support\n~> *!bahasa* untuk melihat list kode bahasa`);
                });
            }
            break
        case `${prefix}checkip`:
            ...
            await limitAdd(serial)
            if (args.length === 1) return aksa.reply(from, 'Kirim perintah *@checkip [ipaddress]*\nContoh : *@checkip 182.0.144.145*', id)
            aksa.reply(from, mess.wait, id)
            arg = body.trim().split(' ')
            console.log(...arg[1])
            var slicedArgs = Array.prototype.slice.call(arg, 1);
...
➸ *Wilayah:* +${region_name}`
            ...
            const base64 = `data:image/jpg;base64,${pictk.toString("base64")}`
            aksa.sendImage(from, base64, city, cekip3)
            } catch (err) {
             console.error(err.message)
             await aksa.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Maaf, User tidak ditemukan')
        ...
        case `${prefix}imgcompress`:
            ...
            await limitAdd(serial)
                    if(isMedia){
                        ...
                        aksa.sendText(from, `Tidak ada gambar! untuk !compress kirim gambar dengan caption !compress`)
                    }
                    async function processImg(gambar) {
                        ...
                        image.quality(55).write('./quote/compressed.jpeg', function (err) {
                            ...
                            aksa.sendFile(from, './quote/compressed.jpeg','compressed.jpg', '_*Processing Sukses!');
                        });
                    }
                    break
    case `${prefix}jarak`:
        ...
        case `${prefix}spek`:
            ...
            try {
                ...
                aksa.sendFileFromUrl(from, resp.data.result.image, 'gsm.jpg', kbbuaww, id)
            } catch (err) {
                console.error(err.message)
                await aksa.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Maaf, tidak ditemukan')
        ...
        case `luluu`:
            ...
            aksa.reply(from, blsn, id)
            break
        case `assalamualaikum`:
        case `asalamualaikum`:
        case `assalamu\'alaikum`:
            if (args.length === 1) return aksa.reply(from, `Waalaikumsalam ${pushname}:)`)
            break
        case `${prefix}hitung`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (args.length === 1) return aksa.reply(from, '[❗] Kirim perintah *!hitung [ Angka ]*\nContoh : !hitung 12*12\n*NOTE* :\n- Untuk Perkalian Menggunakan *\n- Untuk Pertambahan Menggunakan +\n- Untuk Pengurangan Mennggunakan -\n- Untuk Pembagian Menggunakan /')
            const mtk = body.slice(8)
            if (typeof Math_js.evaluate(mtk) !== "number") {
                aksa.reply(from, `"${mtk}", bukan angka!\n[❗] Kirim perintah *!hitung [ Angka ]*\nContoh : !hitung 12*12\n*NOTE* :\n- Untuk Perkalian Menggunakan *\n- Untuk Pertambahan Menggunakan +\n- Untuk Pengurangan Mennggunakan -\n- Untuk Pembagian Menggunakan /`, id)
            } else {
                aksa.reply(from, `*「 MATH 」*\n\n*Kalkulator*\n${mtk} = ${Math_js.evaluate(mtk)}`, id)
            }
            break
        case `p`:
        case `helo`:
        ...
        case `wey`:
        case `woy`:
            if (args.length === 1) return aksa.reply(from, `Hai, *${pushname}!*👋
...
Twitter: https://twitter.com/bdrsmsdn`)
        ...
        case `iri?`:
        case `iri`:
            aksa.sendPtt(from, './media/iri.mp3', id)
            break
        case `abgjago`:
        case `abangjago`:
            aksa.sendPtt(from, './media/bgjg.mp3', id)
            break
        case `tarekses`:
        case `tariksis`:
        case `tareksis`:
        case `tareeksis`:
        case `tareekses`:
            aksa.sendPtt(from, './media/tarekses.mp3', id)
            break
        case `welotka`:
        case `welutka`:
        case `kangcopet`:
            aksa.sendPtt(from, './media/welot.mp3', id)
            break
            //information
        case `${prefix}cuaca`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
        ...
        case `${prefix}owner`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            aksa.sendContact(from, '6281281817375@c.us') //ganti
            break
        case `${prefix}bot1`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            aksa.sendContact(from, '6282115089860@c.us') //bot gue ini
            break
        case `${prefix}bot2`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            aksa.sendContact(from, '6285215130891@c.us') //ini juga
            break
        case `${prefix}bugreport`:
            if (!isRegis) return aksa.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lucya, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
            if (args.length === 1) return aksa.reply(from, '[❗] Kirim perintah !bugreport [teks]\ncontoh : !bugreport Permisi Owner, Ada bug pada command !otakudesu, Tolong diperbaiki')
            const bug = body.slice(11)
            if (isGroupMsg) {
                aksa.sendText(ownerNumber, `*[BUG REPORT]*\n*WAKTU* : ${time}\n*Nama Pengirim* : ${pushname}\n*Group* : ${formattedTitle}\n\n${bug}`)
                aksa.reply(from, 'Masalah telah di laporkan ke owner Lucya, laporan palsu/main2 tidak akan ditanggapi.', id)
            } else {
                aksa.sendText(ownerNumber, `*[BUG REPORT]*\n*WAKTU* : ${time}\n*Nama Pengirim* : ${pushname}\n\n${bug}`)
                aksa.reply(from, 'Masalah telah di laporkan ke owner Lucya, laporan palsu/main2 tidak akan ditanggapi.', id)
            }
        ...
        case `${prefix}restart`: // WORK IF YOU RUN USING PM2
            if(isOwner){
                aksa.sendText(from, '*[WARN]* Restarting ...')
        ...
        case `${prefix}lucyagroup`:
            aksa.reply(from, `➥ *Grup Official Informasi Lucya-BOT I* https://chat.whatsapp.com/LuZkEtgJz4kI6cOkAeHL5j
➥ *Grup Official Informasi Lucya-BOT II* https://chat.whatsapp.com/G0MEd2wMcJhKupMqrWI6fB`, id)
            break
		default:
            if (command.startsWith(`${prefix}`)) {
                aksa.reply(from, `Maaf ${pushname}, Command *${args[0]}* Tidak Terdaftar Di Dalam *${prefix}menu*!`, id)
            }
