...
const stickerlight = (imageUrl) => new Promise((resolve, reject) => {
    fetch('https://api.vhtear.com/lightning?link=' + encodeURIComponent(imageUrl) + '&apikey=' + vhtear, {
            method: 'GET',
        })
        .then(async res => {
            const text = await res.json()
...
const instagram = async(url) => new Promise(async(resolve, reject) => {
    const api = 'https://api.vhtear.com/instadl?link=' + url + '&apikey=vhtear'
...
const igstory = async(query) => new Promise(async(resolve, reject) => {
    const api = 'https://api.vhtear.com/igstory?query=' + query + '&apikey=vhtear'
    axios.get(api).then(async(res) => {
        const sta = res.data.result
        if (sta.status === false) {
            resolve(`Media Tidak Di Temukan`)
        } else {
            resolve(sta)
